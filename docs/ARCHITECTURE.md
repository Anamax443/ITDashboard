...
- **Dashboard** — collector status, summary cards (clickable drill-down, incl. a "Critical services" tile → opens the Critical Services tab, and a "🖨 Náplně/Supplies" tile → opens Printer status pre-filtered to low/empty), health cards (reinstall risk/watch tiles), an expandable "📊 Operating systems" OS-breakdown tile, timeline chart, top noisy PCs chart, top event IDs, computers summary
