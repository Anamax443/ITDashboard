...
- **SMB single-stream (the verdict method).** For each target the service caches an **N-MB random source file on `.213`** (`ensureSource`, sized by `linkspeed.size_mb`), then per cycle **pipelines it up to `\\PC\C$\tmp\itdash-speedtest\<filename>`** (`linkspeed.filename`) and **back down** as a single stream, timing each direction independently, and keeps the **BEST up/down over `linkspeed.cycles` cycles** (the fastest cycle is the link's real ceiling, filtering out transient contention). This is the throughput number the summary classifies (ok/slow/…). When done the **whole client `itdash-speedtest` dir is removed**. **Large-block copy (`SMB_CHUNK` = 8 MB, 2026-07-20).** The pipeline streams use an **8 MB `highWaterMark`** instead of Node's default 64 KB. A single-stream SMB **upload is round-trip-bound on the write side** — with 64 KB chunks each SMB write waits a round trip, capping upload far below the link (the tell was upload measuring ~3× slower than download, e.g. ↑230 vs a real ↑730, while the download — prefetched by the SMB client — and robocopy `/MT` both hit the true link). Copying in 8 MB blocks (what a native copy / robocopy does) makes the SMB upload reflect the link, not Node's stream overhead.
...
- **NIC link speed + machine identity (`readHostInfo`).** Over one **DCOM `CimSession`** (same DCOM stack as the other collectors) it reads two things: (a) the negotiated adapter link rate — mapping the target IP to the adapter via `Win32_NetworkAdapterConfiguration` (IP→interface index) and reading that `Win32_NetworkAdapter`'s reported speed → `nic_mbps` / `nic_name` (a **wireless or non-standard rate is labelled as wifi**); and (b, 2026-07-20) the **machine's own name** (`Win32_ComputerSystem.Name`) — the ground truth for **who actually answered on this IP** (see **Identity** below). This is the link *capacity* (what the NIC negotiated), independent of the throughput a copy actually achieves.

It also pings the target for **RTT (latency)**. Reuses **`tcpProbeTimed`** (TCP/445 precheck) and Windows **`ping.exe`**.

**Identity — attribute the result to the machine that actually answered (2026-07-20).** A powered-off PC keeps its **last IP in `computers.ip_address`** (the reachability collector only ever `COALESCE`s it, never nulls it), and DHCP then hands that IP to a **different** live device — so a batch measuring the IP would otherwise inherit the powered-off PC's name. Fixed on two levels: `resolveEndpoint` resolves an IP-target's name from the **live DHCP lease** (`dhcp_leases`, keyed by MAC, freshest lease wins) first, and only then from the AD inventory; and when a live measurement runs, the **machine's self-reported name** (`Win32_ComputerSystem.Name`, from `readHostInfo`) **overrides** both. The batch also emits a **⚠ activity-log note** when the measured name differs from the inventory name for that IP — a signal that the `computers` row is stale.
...
**Prompt stop (`AbortController`, 2026-07-20).** `POST /system/linkspeed/stop` used to only set a flag checked **between PCs**, so a stop waited out the whole current target (all cycles × N MB + robocopy + DCOM) — tens of seconds. `stopLinkSpeed` now also **aborts an `AbortController`** wired through the run: the in-flight SMB `pipeline` (up and down) takes the `signal` and is cut mid-stream, `robocopy` and the DCOM `powershell` child are **killed**, the cycle loop and the inter-PC `pause` are abort-aware, and the aborted PC returns a **`zastaveno` marker that the batch drops instead of archiving** a misleading slow/error row. `batch.running` therefore flips to done right after the stop rather than after the current PC finishes. The on-demand single-PC test (no signal) and the between-PC behaviour are unchanged.
...
| 076_linkspeed_methods | `link_speed_results.nic_mbps` / `nic_name` / `robo_up_mbps` / `robo_down_mbps` columns — the NIC negotiated link speed (`readHostInfo` over DCOM `CimSession` — same session also reads `Win32_ComputerSystem.Name` for the measured-machine identity, 2026-07-20; wireless/non-standard rate flagged as wifi) and the robocopy `/MT` multi-file figures (informational only — the read-back is cache-inflated), alongside the SMB single-stream verdict. Methods are independently toggled by `linkspeed.method.smb` / `.robocopy` / `.nic` settings |
