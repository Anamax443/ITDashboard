...
- **Service email alerts** — per-PC opt-in (🔔 Services column) for a configurable critical-services list (NTDS, DNS, Kdc, Netlogon, … + wildcards) with a whitelist for noisy updaters; emails when a critical service is non-Running, with flapping protection (time-based debounce + optional maintenance window) so nightly reboots don't page anyone. The same whitelist now doubles as a global view filter — matching services drop out of the Dashboard "Stopped services" tile and (via a default-on toggle) the Services tab counts. Optional **port checks** (phase 2) TCP-probe key infra ports (LDAP 389, SMB 445, RDP 3389, Kerberos 88, DNS 53) from the server to catch "running but unreachable" — testing the whole path network→firewall→OS→service. Shares SMTP/recipients/dashboard URL with disk alerts; off by default.
...
- **Live reachability probe** — a standalone `reachability` check TCP-probes every enabled, non-excluded PC (port 135 → 445 fallback, not ICMP — domain firewall blocks ping) early each checks cycle, independent of monitoring and of the collector failure cap. Drives the Computers **Status** column as live "is this PC on the network now": `Active` = reachable (dim "logs" marker if up but event log unreadable), `Offline` = not reachable, `Disabled` = AD account disabled. New **offline** filter chip/dropdown. Probe runs within the periodic-checks window, so Status is not refreshed overnight/weekends in the default config.
- **OS breakdown chart** — homepage panel charting the live managed fleet by canonical OS bucket (Windows 11/10/Server/…), each bar split into an active and a hatched "stale" (past inactivity threshold) segment; click a segment to drill into the Computers tab filtered to that OS + live/stale.
- **Reinstall-candidate detection** — a second row of dashboard tiles + a candidates panel that ranks PCs by accumulated eventlog problems to surface boxes due for a reinstall ("chronic error accumulation = probably due for a reinstall"). A "damped blend" score over a configurable window caps per-signature occurrences (so one chatty driver can't flag a healthy box), weights severity, and rewards breadth (many *different* errors) and persistence (errors across many *days*) — catching a systemically sick box, not just a noisy one. Tiles drill into the Computers tab filtered to the risk/watch PC set.
...
**Live reachability probe (commit `a12ad36`):** the Computers **Status** column used to be a by-product of the eventlog collector — it ran only on enabled + monitored + non-excluded PCs, classified failures crudely (offline / rpc_unavailable / access_denied / unknown), and FROZE once a PC passed the failure cap, so a reachable box with an unreadable event log showed "Unknown" and an AD-enabled box never classified got a green "Active" fallback. It never answered "is this PC on the network now"; there was no ping. New standalone probe `apps/server/src/services/reachability-collector.ts` does, for EVERY enabled && !excluded PC (independent of monitoring and of the failure cap), a plain TCP connect to port 135 (RPC endpoint mapper) with fallback to 445 (SMB) — NOT ICMP ping, which Windows Firewall blocks by default in a domain; first port that answers = reachable. Runs as a new `reachability` check in checks-runner, early each checks cycle, recording `reachable` / `last_reachable_at` / `reach_checked_at`. Status now means live reachability: `Disabled` = AD account disabled; `Active` = reachable now (secondary dim "logs" marker when up but event log unreadable — permissions/RPC); `Offline` = not reachable (powered off / disconnected). Until the first probe `reachable` is null and the column falls back to the old `last_status` with a neutral "— not probed". New **offline** filter (status chip + dropdown) = enabled && !excluded && reachable false, plus a Settings toggle for the probe. Migration 032 adds the three columns + settings `checks.run_reachability=1`, `reachability.ports=135,445`, `reachability.timeout_ms=2000`. Caveat: the probe runs within the periodic-checks window (default Mon–Fri 06:00–18:00), so Status is not refreshed overnight/weekends in the default config.

**Reinstall-candidate / faulty-PC detection (commit `31347b0`):** a second row of dashboard tiles + a candidates panel that ranks PCs by accumulated eventlog problems to surface reinstall candidates — the premise being that a PC with chronic error accumulation is probably due for a reinstall. New server endpoint `GET /events/pc-health`. Over a window (default 14 days, configurable) it computes, per enabled && !excluded PC, a **"damped blend"** score: for each distinct signature (`provider`+`event_id`+`level`) it caps occurrences at `signature_cap` (default 20) so one chatty source (e.g. a driver spamming thousands of events) can't flag a healthy box; weights severity critical×10 / error×3 / warning×1; adds a **breadth bonus** (count of DISTINCT error/critical signatures ×5) and a **persistence bonus** (DISTINCT days with errors ×3). `score = Σ min(cnt,cap)·weight + signatures·5 + days·3`. Classified **risk** (≥ `threshold_risk` 150) / **watch** (≥ `threshold_watch` 60). Second-row tiles "🩺 Reinstall candidates" (risk count) and "Watch" (watch count) drill into the Computers tab filtered to that PC set (removable chip). The candidates panel lists the worst PCs with score + crit/err/warn + distinct error types + active days, each clickable to jump to the PC. New Settings block (window days / per-event-type cap / watch & risk thresholds); severity weights DB-tunable. Migration 033 seeds all knobs as tunable settings. Client fetches on a slow 5-min cadence (heavier 14-day query). Rationale: raw event volume misleads (one noisy driver); the cap + breadth/persistence captures a systemically sick box (many DIFFERENT errors across many DAYS).
