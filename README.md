...
- **Problem-PC detection** — a second row of dashboard tiles ("Problem PCs" / "Watch") that rank PCs by accumulated eventlog problems to surface boxes in trouble. A "damped blend" score over a configurable window caps per-signature occurrences (so one chatty driver can't flag a healthy box), weights severity, and rewards breadth (many *different* errors) and persistence (errors across many *days*) — catching a systemically sick box, not just a noisy one. Clicking a tile expands an inline breakdown table (score · crit · err · warn · types · days); each row jumps to that PC.
