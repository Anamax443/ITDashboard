...
const dict = {
  cs: {
    ...
    'settings.field.mikrotikHelp': 'Sběr běží skriptem na SQL serveru (10.8.2.225) každých 5 min — odtud má MikroTik povolený přístup. Heslo se ukládá šifrovaně (ne v plaintextu); maskované „••••" znamená, že je nastavené — přepiš ho jen když ho chceš změnit. IP routerů a uživatele měň tady, skript se nesahá.',
  ...
  en: {
    ...
    'settings.field.mikrotikHelp': 'Collection runs as a script on the SQL server (10.8.2.225) every 5 min — that host is allowed on the MikroTik. The password is stored encrypted (never plaintext); a masked "••••" means one is set — only overwrite it to change it. Edit router IPs and the user here; no need to touch the script.',
    'settings.section.perfLookback': 'Perf-events lookback',
