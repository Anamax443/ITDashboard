...
const dict = {
  cs: {
    ...
    'settings.field.svcAlertsHelp': 'Sledovaná PC se zaškrtávají v Počítačích (🔔 Služby). Alert se pošle, až je služba mimo Running déle než debounce (ochrana proti flappingu při nočních restartech) a mimo okno údržby. Sdílí SMTP/příjemce/adresu dashboardu s diskovými alerty. Test uloží rozpracované nastavení sám.',
  ...
  en: {
    ...
    'settings.field.svcWhitelistPlaceholder': 'gupdate*, GoogleUpdater*, edgeupdate* …',
    'settings.field.svcAlertsHelp': 'Watched PCs are ticked in Computers (🔔 Services). An alert fires once a service has been non-Running longer than the debounce (flapping guard for nightly reboots) and outside the maintenance window. Shares SMTP/recipients/dashboard URL with disk alerts. The test saves pending edits first.',
