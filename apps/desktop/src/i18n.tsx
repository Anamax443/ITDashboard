...
const dict = {
  cs: {
    ...
    'settings.field.scanRanges': 'Rozsahy ke skenování (Lokalita=CIDR, na řádek nebo čárkou)',
    'settings.field.scanHelp': 'Aplikační server pinguje IP v zadaných rozsazích, které router nezná, a z vlastního ARP zjistí MAC — tím chytí i staticky adresovaná zařízení (např. tiskárnu se statickou IP nastavenou přímo na ní), která router nevidí. Skenuje jen IP, které ještě nemáme s MAC, takže postupně obsáhne celou síť. Běží na stejném intervalu jako sběr.',
  ...
  en: {
    ...
    'settings.field.scanRanges': 'Ranges to scan (Site=CIDR, one per line or comma-separated)',
    'settings.field.scanHelp': 'The app server pings IPs in the given ranges that the router does not know and reads its own ARP for the MAC — catching statically-addressed devices (e.g. a printer with a static IP set on the device) that the router never sees. It only scans IPs we do not already have with a MAC, so it progressively covers the whole network. Runs on the same interval as collection.',
