...
const dict = {
  cs: {
    ...
    'actions.adminUserHint': 'ITD_ADMIN_USER env variable (User Environment Variables) má 3 módy: (1) unset = launchery běží jako tvůj přihlášený účet (typicky bez admin oprávnění na cíli); (2) konkrétní hodnota (např. AXINETWORK\\trnka_admin) = každý launch wrapne v runas /netonly, Windows credential dialog s předvyplněným userem žádá jen heslo; (3) hodnota "ask" = pro každý launch CMD nejprve vyzve k zadání admin účtu (prázdné poprvé, příště pre-fill posledně zadaného z %LOCALAPPDATA%\\ITDashboard\\launchers\\last-admin-user.txt — Enter potvrdí), pak Windows credential dialog na heslo. Heslo se NIKDY nepamatuje. Mód "ask" je správný pro multi-admin workstation (více IT specialistů sdílí PC).',
  ...
  en: {
    ...
    'actions.adminUserHint': 'ITD_ADMIN_USER env variable (User Environment Variables) has 3 modes: (1) unset = launchers run as your logged-in account (typically without admin rights on the target); (2) a concrete value (e.g. AXINETWORK\\trnka_admin) = every launch wraps in runas /netonly, the Windows credential dialog has the user pre-filled and asks only for the password; (3) the value "ask" = each launch first prompts in CMD for the admin account (empty the first time, pre-fills the last entered user from %LOCALAPPDATA%\\ITDashboard\\launchers\\last-admin-user.txt on next runs — Enter confirms), then the Windows credential dialog for the password. Password is NEVER remembered. The "ask" mode is the right pick for multi-admin workstations (several IT specialists share the PC).',
