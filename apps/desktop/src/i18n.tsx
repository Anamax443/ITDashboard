...
const dict = {
  cs: {
    ...
    'settings.field.diskAlertsHelp': 'Sledované PC se zaškrtávají v záložce Počítače (sloupec 📧 Disk). U každého lze vyplnit konkrétní písmena disků (např. C nebo C,F); prázdné = všechny in-scope disky. Vyhodnocují se podle kritického prahu z nastavení Disky výše. Před testem ulož nastavení.',
  ...
  en: {
    ...
    'settings.field.diskAlertsHelp': 'Watched PCs are ticked in the Computers tab (📧 Disk column). Each can specify drive letters (e.g. C or C,F); empty = all in-scope drives. They are evaluated against the critical threshold from the Disks settings above. Save settings before testing.',
