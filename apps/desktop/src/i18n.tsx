...
const dict = {
  cs: {
    ...
    'services.help.actions': 'Filtry (kombinují AND): Hide trigger-start (default ON), Hide delayed-start, Hide per-user (default ON), Hide compliant, ⚠ Only ExitCode != 0. Sloupec Exit ukazuje Win32 ExitCode služby — 0 = graceful (typicky trigger-start co dokončil úlohu a vypnul se), != 0 = SCM-level crash (zobrazí se červeně). 🔧 Scan services = manuální scan. 📤 GPO script = stáhne PowerShell pro hromadnou opravu přes GPO startup script.',
  ...
  en: {
    ...
    'services.help.actions': 'Filters (combined with AND): Hide trigger-start (default ON), Hide delayed-start, Hide per-user (default ON), Hide compliant, ⚠ Only ExitCode != 0. The Exit column shows the Win32 ExitCode of the service — 0 = graceful (typically a trigger-start service that finished its task and shut down), != 0 = SCM-level crash (shown red). 🔧 Scan services = manual run. 📤 GPO script = downloads PowerShell for fleet-wide remediation via GPO startup script.',
