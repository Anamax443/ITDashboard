...
**D) What is NOT verified.** The live DCOM/HKU read under the service account. Rights are confirmed and the generated PowerShell is parser-checked plus unit-tested for escaping (TS template literal → PS → regex is silent when wrong: PowerShell doesn't treat `\` as an escape, so a malformed path never throws — it just never matches, and every PC would report "clean"). The binary decode **is** verified against a real captured `DisabledItems` value: 330 bytes → correct .dll path, add-in name, and NAV detection. First run with `officeaddins.enabled=1` is the remaining test.
