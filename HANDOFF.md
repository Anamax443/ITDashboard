...
Last updated: 2026-07-20 (**full doc sweep — README, ARCHITECTURE, dashboard.html, nacitani-mac-hostname.html, project-status.html, presentations CS+EN, in-app deck, HANDOFF all current**; migrations 001–080 (this change adds none); LIVE `2b558ee`). **Newest (2026-08-03, code on `main`, NOT yet deployed — needs migration 081 + service restart on .213):** **Precise interactive logon/logoff history from the Security event log (4624 logon / 4634 logoff), paired by LogonId.** The existing "Historie přihlášení" was WMI-only — a snapshot of `Win32_ComputerSystem.UserName` sampled each collector cycle, so it misses logins between scans and has no exact logon/logoff time. Verify-core on a live PC (svc-itdashboard → TRNKAMW11) confirmed the prerequisites hold: **Audit Logon = Success is on**, the account **can read the Security log** remotely, and interactive logons are present — but also that **~99 % of 4624 volume is noise** (type 3 network + type 5 service; only 2 of 200 were interactive). So this is a **separate collector**, not a flag on the System/Application eventlog collector: new `logon-collector.ts` pulls ONLY interactive logon types (2/7/10/11) **filtered server-side in the `Get-WinEvent -FilterXPath`** so the type-3/5 flood never leaves the target, extracts `TargetUserName/Domain/LogonType/IpAddress/TargetLogonId` from the event XML, and pairs 4624→4634 by LogonId into new table **`pc_logon_events`** (migration **081**, own cursor `computers.last_logon_collected_at` kept separate from the eventlog collector's `last_collected_at`; `tryWithHostLock` so a busy PC is skipped and retried). It's a **pure add-on** — never touches `reachable`/`consecutive_failures`. New check `checks.run_logon` (default on) after `eventlog`; also added to per-PC refresh (step 6) and the nightly retention (`sp_purge_pc_logon_events`, `pcLogonHistory.retention_days`=90). The **UserHistoryModal** now shows two sections: **"Přesná přihlášení (z eventlogu)"** (precise, with logon/logoff/duration/type, "probíhá" for open sessions) above the existing **"Snímky přihlášení (odhad)"** WMI table, with a plain note when the precise data is empty (audit off) — graceful degradation, never an error. **Verify-core done for prerequisites; the exact FilterXPath has NOT yet been run end-to-end against a real box** (built after scope approval) — confirm on first deploy (test command in the handoff). typecheck + build (server+desktop) + 86 server tests all green. See "Session 2026-08-03" below. **Prev (2026-07-20, LIVE `2b558ee`):** **Random / locally-administered MAC flag (🎲) in the device matrix.** Windows 11 and phones randomize their Wi‑Fi MAC per network (a new, non-hardware address), and virtual NICs use locally-administered addresses — such a MAC carries no real vendor OUI and is **not a durable device identity**. New **pure** helper `isRandomMac(mac)` (desktop `api.ts`) tests the U/L bit (`0x02`) of the first octet; the Devices matrix now shows a **🎲** marker with an explanatory CS+EN tooltip on those rows. **Client-only — no migration, no collector/server change:** a randomized MAC has the local bit set so it can never collide with a real vendor OUI, so `suggestCategory` and AD-pairing already behave correctly; **wired desktops keep their real MAC and are unaffected**. 6 new unit tests, `api.test.ts` 45/45 green; `tsc --noEmit` + `vite build` clean. **Prev (2026-07-16, LIVE `ba999ab`):** **Disabled Office add-ins are now monitored per PC (migration 078).** Office silently disables an add-in after it crashes (`Resiliency\DisabledItems`) and reports this NOWHERE — no Event Log entry, no dialog; the app just looks healthy while quietly not doing its job. Found the hard way: a disabled **NAV Excel add-in** made "export to Excel" from Navision open an empty workbook — NAV never ships a finished table, it writes a bare `.xltx` template carrying only the connection (Server/Port/ServerInstance) and the add-in fetches the rows over OData. It worked for a colleague on the same Microsoft 365 because `DisabledItems` lives in **HKEY_CURRENT_USER — per user, not per Office version**; Office version and bitness are red herrings (64-bit Office vs the 32-bit NAV client is supported, the add-in DLL is AnyCPU, and the `.xltx` association is fine). New collector `office-addins-collector.ts` reads the remote registry via **WMI StdRegProv over a DCOM CimSession** (plain WinRM is not configured on domain PCs) against **HKEY_USERS**, walking logged-on users x Office versions x Excel/Word/Outlook/PowerPoint; the REG_BINARY payload is undocumented, so readable UTF-16 runs are extracted and the .dll path + add-in name pulled out of them (verified against a real captured value). Cheap TCP/135 pre-flight, then `tryWithHostLock(pc:<id>)` so a busy PC is skipped and retried next cycle. New tables `office_addin_scans` + `office_disabled_addins` (current state, rewritten per scan — an add-in can be re-enabled and must then disappear), settings `officeaddins.enabled` (**off by default**) / `officeaddins.interval_sec` (6 h — this state changes on a crash, not by the minute), endpoints `GET /office-addins`, `GET /office-addins/status`, `POST /office-addins/scan`, a 🧩 column on Computers and a 🧩 homepage tile. **Honest limitation, by design:** `DisabledItems` is readable only for **logged-on** users — `NTUSER.DAT` is exclusively locked while the profile is loaded, so the offline C$ route that crash-dump collection relies on does NOT work here (verified, not assumed). A PC with nobody logged on scans as `no_users` = **unknown, not clean**, and both the column and the tile keep those out of the OK count. Coverage therefore fills in gradually as people sit at their machines. **Verified live (2026-07-16, first manual scan):** `pcs=93 scanned=65 withIssues=22 navDisabled=0`. The DCOM/HKU read works under `svc-itdashboard` exactly as designed. **But that first run also proved the decode wrong twice, and migration 079 fixes both** (see below) — treat the `withIssues=22` figure as inflated; it counted disabled *documents* as add-ins. `navDisabled=0`: the reporting workstation was the only NAV case and it was already fixed. `no_users` showed up on 19 PCs in the same run, confirming the logged-on-only limitation is a real, visible everyday condition, not a theoretical caveat. The only `error` results were the **domain controllers** (`B-S-W-DC-01/02/03`) — the service account is not admin there, the same expected failure disk/link-speed collectors already document. **Prev (2026-07-01, LIVE `1687930`):** **Link-speed scheduler is now ROLLING, not a one-shot sweep.** Previously the scheduler ran one big `all` batch every `linkspeed.interval_hours` (default 24 h). Now, on **every tick** (`linkspeed.tick_min`, default 20 min) it measures just a **few of the stalest due PCs** (`linkspeed.batch_size`, default 6) whose last measurement is older than the refresh window — the load is **spread out** (no hourly burst, something is always being measured) and the scheduler **idles** once every PC is fresh. `linkspeed.interval_hours` is **repurposed to "refresh each PC every X h"** (per-PC freshness, may be **< 1 h**). Freshness is read from the DB (`MAX(measured_at)` over `COALESCE(ip_address, target)`), so it is **robust across service restarts** — the old boot-reset of `lastSchedRunMs` on every deploy meant the sweep never fired. `link-speed.ts` gained `pickDueTargets` (stalest-N due) + a rolling loop; new settings `linkspeed.tick_min` / `linkspeed.batch_size` (plain settings keys, **no migration** — migrations stay 001–077). Run conditions: `linkspeed.enabled` + targets + hour window. i18n CS+EN. **Prev (2026-07-01 pokračování, LIVE `848da26`):** **Link-speed module — deep rework + new mechanics.** A page-level **"Cyklů" field** (empty = from Settings, `1–20` overrides the batch AND per-PC test) and a **"Ignorovat výjimky z Nastavení"** checkbox for MANUAL runs only (scheduled runs always honour exclusions). A **run selector** (Poslední běh / Poslední 3 / Poslední 10 / Všechny běhy) where **each batch = one run** stamped with a `run_id` (= batch start time, mig 077) that drives the summary, the networks panel and the table. **↺ Reset souhrnu is NON-destructive** — it sets a baseline (`linkspeed.baseline_at`), never DELETEs, so history stays. **Three measurement methods** (Settings → "Metody měření", each toggleable): **SMB copy** (default ON, single-stream ↑↓ — **the verdict is computed from THIS only**, matching what a user feels on one file), **Robocopy /MT** (default OFF, multi-file parallel — read-back is often **cache-inflated** and can exceed the port speed, e.g. 2800 Mb/s on a 1 Gb port, so the **RC column is informational only and never decides the verdict**), and **NIC port speed** (default ON — negotiated link speed via **DCOM CimSession**, the same path as the disk collector, NOT WinRM; `<1000` = 100-Mb port, non-standard rate or "wireless/Wi-Fi" adapter name = wifi). **Wifi is its own category** (blue "wifi" chip, never judged by the wired threshold, excluded from the "100Mb port" alarm). **IP + hostname are always on the output** (mig 075: hostname target → forward-DNS to the current IP so it can tell wifi from cable; IP target → name from inventory). **Sítě (/24) panel** with a per-network verdict (systematically-slow network / individual PCs / wifi / "0 měřeno" = in inventory but unmeasured, hatched red), APIPA/loopback filtered; click a network to pre-fill Cíle. **HTML report opens in a NEW TAB** with "Co kontrolovat" + counts (incl. wifi) + network bars + a detail table sorted by IP; CSV/HTML/PDF export. **`all` now = AD computers** (`computers` WHERE `enabled=1 AND excluded=0 AND ip_address NOT NULL AND (reachable=1 OR (reachable IS NULL AND consecutive_failures<10))`), NOT the DHCP/UniFi inventory (relaxed from the old hard `reachable=1`). **Per-host lock** (`services/host-lock.ts`) serializes heavy per-PC ops (SMB C$, WMI, robocopy) **on the computer IDENTITY** (`pc:<id>`, not the dynamic IP): `withHostLock` blocks (link-speed), `tryWithHostLock` skips busy PCs (crash-dump); different PCs still run in parallel. **⟳ Restart služby** button in Settings (`POST /system/service/restart`, detached process → `net stop/start ITDashboardAPI`, verified live). Migs **075** (`ip_address`,`host_name`), **076** (`nic_mbps`,`nic_name`,`robo_up_mbps`,`robo_down_mbps`), **077** (`run_id`); new settings `linkspeed.method.smb/robocopy/nic`, `linkspeed.pause_ms`, `linkspeed.baseline_at`; new endpoints `/system/linkspeed/reset`, `/system/service/restart`, and `/linkspeed/run` takes `cycles` + `ignoreExclusions`. See "Session 2026-07-01 (pokračování)" below. **Prev (2026-07-01, LIVE `d7d6890`):** **Link-speed / connection-quality measurement — a "Měření linky" (⚡) tab.** The app server writes an N-MB random file to a live PC's `C$\tmp\itdash-speedtest` over SMB (upload) and reads it back (download), computing **up/down Mb/s** from the wall time + a **ping RTT (latency)** — real link quality that catches a **bad cable / 100-Mb port / congested segment** on the 1 Gb network (both ends are real machines, so it's NOT router-CPU-bound like the WAN `/tool/fetch`; reuses the existing admin-share `C$` access). Each measurement runs **`linkspeed.cycles`** cycles (default 4) and keeps the **best** up/down (transient AV/CPU dips don't understate capacity) — same shape as the operator's old PowerShell script (`services/link-speed.ts`). **Targets**: IPs/hostnames (one per line), a range `10.8.2.*` / `10.8.2.180-182`, or **`all`** (IPs of active PCs/NTB/servers — printers excluded). Runs a **background batch** (SMB-prechecked on 445, dead IPs skipped fast); a target that's **offline** (no ping) is labelled `offline` (dim, not measured) vs `SMB/445 blokováno` (up but blocked) vs `EPERM` (servers/DCs — the svc account isn't admin there). Results **archived to `link_speed_results`** (migrations **071–074**: table + `latency_ms` + `cycles` + settings) and shown as a **sortable grid** with **per-column filters** (numeric `>N`/`<N`/`a..b`, date ranges), a global all-column search + a status filter; the **target IP links to Computers**, a **Hostname** column is resolved from the inventory, plus **Latency** + **Cycles** columns. **Exports** CSV / standalone HTML / print→PDF (of the filtered+sorted view). A **live cmd-like console** streams the running batch (colour-coded, auto-scroll), and every batch **logs to `activity_log`** (source `linkspeed`: start / per-result / finish) so it shows in the Aktivita feed. A **⚡ Linky homepage tile** (`GET /system/linkspeed/summary`) shows the slow-link count over the latest-per-target; the page shows a **visual overview** (count chips + bars of the slowest links). **Endpoints:** `POST /system/linkspeed/{test,run,stop}`, `GET /system/linkspeed/{status,history,summary}`. **Settings** section "Měření linky (plán)": enable, interval (h), **hour window** (`linkspeed.window_start/end` — scheduler runs only within it, restart-safe), targets, **hostname exclusions** (`linkspeed.exclude_hosts`), cycles, **test file name** (`linkspeed.filename`=`itdash-speedtest.tmp`), size, OK threshold (`linkspeed.ok_mbps`=200, verdict by the WORSE direction). **The whole test dir is always deleted from the client afterwards.** **AV note:** no extension/archive (zip/rar/nonsense) dodges on-access scanning (ESET reads content) — the only fix is an **ESET path exclusion** for `C:\tmp\itdash-speedtest` (or the configured filename). **Live finding:** a full-fleet run surfaced ~25 slow links incl. **ZAS-SQL-01 at 12.7 Mb/s** and 10.180.90.237 at 9.8 — classic bad cable / 100-Mb port. **Previous (2026-06-30 late, LIVE `d55552d`):** **Per-branch service-port matrix + service discovery — BUILT then PARKED.** A **"Porty poboček"** tab (`GET /system/service-ports`, `services/service-port-matrix.ts`, `ServicePortsMatrix.tsx`) showed a **site × service-port matrix** (default printers 9100/515/631): for each `Label:port:category` check the app server **probes that TCP port on every categorized device of that category at each site, from .213 over the WAN**, aggregating **open/online with offline counted separately**; clicking a cell **lists the actual devices** (open/closed/offline, IP+name). The matrix is **OUI-aware** — `svcports.voip_ouis` promotes Yealink/VoIP devices to an effective `voip` category. Plus a user-triggered **service discovery scan** (`POST /system/service-discovery`, `services/service-discovery.ts`): scans a broad TCP port set (well-known 1–1024 + common highs, or full 1–65535) on a **sample of devices per category** to learn the real open-port profile (so the check list reflects what actually runs, not a guess). Migrations **065–070** seed `svcports.*` / `svcdisc.*`. **PARKED (current state):** the nav tab is hidden and `svcports.enabled=0` (mig 070) so the 15-min probe sweep is stopped — **all code/endpoints/view/component stay** (revive = restore the nav button + `svcports.enabled=1`). **Why parked:** the matrix probes from the central .213, so it answers "can the **CENTRE** reach device X", **not** "can the **BRANCH** reach it" — and the real question ("do branches print to the central / printer segment?") must be measured **from the branch router**, not .213. Cross-site rows (HQ IPs under a branch) are **not a bug** — branches route to HQ to print. Also the org is **mid SD-WAN redesign separating the printer network**, so the target (printer-segment subnet) is a moving target. Revive after SD-WAN as **per-branch reachability to the printer segment, measured from the branch routers** (MikroTik `netwatch type=tcp` / `/rest/ping`; note `dhcp-reader` is read-only, so a test/netwatch policy bump may be needed — like `/tool/fetch` needed admin). **VoIP finding:** the IP phones are **Yealink** (OUI 24:9A:D8) on a **T-Mobile cloud PBX** (BroadWorks, `xspweb.t-mobile.cz`); they are **not in the device inventory** (separate voice network) and unreachable from .213, so **VoIP monitoring is out of scope from the central server** — authoritative phone status lives in the T-Mobile portal/API. (Memories `itdashboard-svcports-matrix`, `itdashboard-voip-out-of-reach`.) **Previous (2026-06-30, LIVE `e0db733`):** three additions. **(1) Dashboard "Komunikace" health tile** — new `GET /system/comms` aggregates SQL, the MikroTik REST API, FTP downloads, the EventLog collector, e-mail (SMTP/M365) and UniFi into one overall OK/degraded/down; a **📶 Komunikace** tile + a collapsible `CommsHealth` panel show each channel (status dot, detail, last error, last-OK). A channel is OK unless its last attempt errored (`activity_log` latest level=`error`); FTP also fails on stale/missing per-site data; unconfigured channels show "vypnuto" and don't count. **(2) WAN monitor** — `wan-monitor.ts` (scheduled, DB-driven enable/interval) pings each branch router (`mikrotik.routers`) + one internet target (`wan.internet_target`=1.1.1.1) **from .213** every `wan.interval_sec`=60 → latency + packet loss, **current snapshot only** (no history); `GET /system/wan`; an always-visible **"📡 Komunikace na pobočky a internet"** dashboard strip with a colour-coded chip per site (green/amber/red, thresholds `wan.latency_warn_ms`=80 / `wan.loss_warn_pct`=5). Plus an opt-in central **download speed test** (OFF by default) — N **parallel** streams (`wan.speedtest_streams`=6, like fast.com) of a configurable file (`wan.speedtest_url`=Cloudflare `__down`, 25 MB), timed **from the first byte** (excludes handshake/TTFB), own long cadence (`wan.speedtest_interval_sec`=1800); shown as "↓ X Mb/s" on the Internet chip; measures the internet **from the server**, not per-branch. Migrations **062–064**; Settings section "Komunikace na pobočky a internet". **Honest finding (verified live):** per-branch internet speed via the MikroTik **`/tool/fetch`** is NOT viable — the Zastavka hEX did 24 MB / 51 s = **~3.9 Mbps**, which is the router's CPU/TLS ceiling, not the link — so branches keep **latency+loss only**; true branch capacity would need a download from a real PC at the branch (memory `itdashboard-wan-monitor`). **(3) Pády (crash) page** — new `GET /crashes/status` strip (**🛰 poslední sběr** · **💾 poslední zápis do SQL** = `MAX(crash_dumps.ingested_at)` · **⏭ další sběr** derived from the live `crash.interval_sec`, or "sběr vypnutý"); the printable **"Analýza pádu" report** is now **LIGHT by default** with a **◐ light/dark toggle** (hidden on print; print always light) and a **UTF-8 BOM** (no more `AnalÃ½za` mojibake when opened from disk); and the explanation is now **derived from the dump** — plain "Co se stalo" + "Technický rozbor" + "Doporučený postup" that distinguishes a Windows-kernel (`nt`) fault from a third-party driver, and for **corruption-class** bugchecks (0x13A/0x19/0xC2/0x139/0x1A) **no longer exonerates drivers** (all-`nt` stack there only marks where corruption was *detected*, not caused → recommends **Driver Verifier + memtest**); same explanation now also shown in the live detail panel. **Previous (2026-06-29, LIVE `4d8ef04`):** **BSOD / crash-dump collection + analysis** — ITDashboard now pulls kernel **minidumps** from monitored, reachable PCs (`\\PC\C$\Windows\Minidump\*.dmp`, dedup by computer+filename, never `MEMORY.DMP`) into the DB (mig 061 `crash_dumps`, raw `.dmp` as a blob, `status` pending→analyzed→failed), and a **separate analyzer worker** runs **cdb** (Debugging Tools installed on .213) to extract the diagnosis. The newest cdb's headless `!analyze` is broken (gallery async), so we use a **sync recipe** `.bugcheck; kc; lm; !dpcwatchdog; .thread; !thread; !process -1 0` which yields STOP code + bug-check name + hot function + the **offending process** (proven live: ZAST23W10 0x133 = `msedgewebview2.exe` closing a large section, `nt!MiDeleteSubsectionPages` 97/97, no third-party driver). New **💥 Pády** tab (aggregations: STOP breakdown / top culprit / repeat offenders + table + per-crash detail with full cdb output + ⬇ `.dmp` + a **white printable "Analýza pádu" report** like Souhrn/Prezentace) + a **💥 dashboard tile** + a **Settings → "Sběr pádů (BSOD)"** section. The service account `svc-itdashboard` reads client `C$` via its **Server Admins** membership (no privilege escalation). **OFF by default** (`crash.enabled=0`). Verify-core was done live end-to-end (transport, blob write+readback, analysis) before building. **Previous (2026-06-26 evening, LIVE `2f74eac`):** **customizable dashboard tile layout** — an operator edit mode (✏️) places each homepage tile by **column letter + row number** (e.g. `B3`) at **two sizes** (full / half), can **stack two half tiles** in one cell via an **upper/lower slot**, and gives each tile row an optional **heading**; layout + headings persist in settings (`dashboard.tile_layout` / `dashboard.row_titles`), re-synced on load so a refresh restores them. The **"PC v problémech" and "Operační systémy" tiles are now first-class grid tiles** (placeable/sizable); their detail panels render below and were fixed so they no longer collapse in the flex view. Also shipped: the **Retention-policy table gained a per-row manual run** (▶ per row + Označit/Odznačit/Spustit vše) — the 3 device-inventory tables (`dhcp_leases` ghosts, `device_ip_history`, `device_ping_samples`), previously only pruned inline by the collector, are now discrete runnable steps; **Computers — Monitor and Exclude are mutually exclusive** (enforced server-side on per-row + bulk endpoints). **Speed-measurement investigation (parked):** per-IP throughput can't be read from the Brno MikroTik (it's a side DHCP device, not the client gateway 10.8.2.1; `/ip accounting` was removed in RouterOS v7) — the clean path is **per-PC NIC counters via the existing CIM/DCOM channel** (`Win32_PerfFormattedData_Tcpip_NetworkInterface`). **Earlier (2026-06-26 late, LIVE `479b7f4`):** **presentation is now a first-class in-app tab** (🎞 Prezentace, CS/EN, clean Print/PDF + Save HTML via the Manager-Summary serialiser, deck CSS scoped under `.deck`); **device history search** — `device_ip_history` gained `host_name` + IP/host indexes (mig 059); `GET /devices/history?q=` matches IP / MAC / hostname; a search panel on the Routers page; own retention `devices.history_retention_days` (default 365). **Retention audit** found two unbounded-growth tables — `perf_events` and `ad_sync_runs` had no purge — now closed (mig 060: `sp_purge_old_perf` / `sp_purge_ad_sync_runs` + `perf.retention_days`=180 / `adsync.runs_retention_days`=90, wired into the daily retention runner). Settings gained a **central 🗂 Retention-policy overview table** (`#sec-retention`) listing every growing table with an inline keep-duration, plus a reusable `sectionLink()` deep-link (wired from MikroTik). **OPEN for next thread:** Notifications/mailing overview table (same pattern as retention), clickable Souhrn drill-through (+ PC-vs-notebook split by name suffix `n`/`ntb`), router-authoritative IP + "router ≠ AD/DNS" flag on Computers, screenshot-per-deploy watcher. **Earlier 2026-06-26 (LIVE `0c86ee9`):** **"Routery" page** (per-router FTP status cards + ⬇ "Stáhnout teď" → forced FTP fetch with a terminal console + live `dhcp_leases` listing with full-column filter / sort / pagination / CSV export) + **📡 Routers homepage tile**; Settings **FTP controls** (`mikrotik.ftp_enabled` / `ftp_sites`) + **block filter** + **new-PC defaults** (mig 058: disk / services / crit-services / excluded, INSERT-only); fixed **stale homepage device tiles** (printers/devices/loss/ports/disks/services now in the 30s refresh) + **`/docs/<file>` serving** (the presentation was 404) + 🎞 Presentation topbar link + 💾 Save HTML on the deck; **data authority model** recorded (router = 1st for IP/presence, AD = 2nd for identity, operator comment = override). See "Session 2026-06-26" below. **Prior (batch 3, LIVE `0cb377d`):** MikroTik **FTP file source** (router writes `IP_scan.txt`+`ARP_scan.txt`, app pulls over FTP and merges by MAC — file is now the primary lease source, REST kept as supplement for ip-scan names) + **per-site data-freshness / availability e-mail alert** (timezone-safe via `file_changed_at`; branches muted by default) — see "Session 2026-06-25 (batch 3)" below. Newest work (2026-06-24/25) — **identity & "real counts" hardening** (see "Session 2026-06-24/25" below): device→AD pairing by **static IP only** (+ shared printers by host name in `comment`); **store the IP the probe actually reached** (TCP peer / ICMP `TTL=` reply), not a stale forward-DNS A record (fixed VOTAVAPW11N showing a dead .181.3.51 while alive at .165); **reverse-DNS scan naming** (NetBIOS firewalled from `.213`, but reverse DNS doesn't resolve from `.213` either — so a nameless AD-matched scan row shows the **matched AD name** in Hostname instead); **auto-confirm category for AD-matched machines** (identify once via AD → stored by MAC → reused); **bulk "confirm suggestions"** button; **IP-address archive per MAC** (mig 055 `device_ip_history`, 🕓N badge → click shows every IP a device used with first→last seen); **identified devices are never auto-deleted** (stale-lease prune + shared-printer prune skip a row with a confirmed category/name/note — except phones); **DB-identity surfacing** (a confirmed-category device with no live row is shown offline via a UNION on `device_categories`, so counts reflect identity — pc/server/phone excluded; phones use randomized MACs); **UniFi honours `!`/`<>` exclude ranges** (private-device VLAN won't flicker back); **CI smoke test fixed** (cmd `findstr` match — the old `if "%ACTUAL%"=="…"` false-failed on the quoted JSON SHA; PowerShell blocked by AllSigned); **shared printers keyed by host NAME** not IP (an IP-based key duplicated a printer on every host DHCP change); **router REST halved** (scan reuses Phase-1 ARP). **Ctrl+↑/↓ moves a line** in the scan-ranges textarea. **Session 2026-06-25 batch 2 (LIVE `b370537`):** the WIP Souhrn page is now **wired + shipped** as a white, theme-independent managerial document (Tisk/PDF **+ Uložit HTML** standalone snapshot); legacy `SHR-<ip>-*` duplicate printer rows are **cleaned up self-healingly** (identity migrated to the host-name key, orphaned `device_categories` purged — real printer count 52→49); and the collector gained a **3rd per-router REST source — `POST /rest/tool/ip-scan`** (NETBIOS/DNS/SNMP names, run ON the router so NetBIOS resolves; gated `mikrotik.ipscan`=on / `mikrotik.ipscan_duration_sec`=10). **All 4 routers are live** (Brno 259, Zastavka 103, Jihlava 34, Svitavy 15 + UniFi). **Honest finding:** ip-scan names only where the network publishes them — Brno (CHR) returns ~256 NETBIOS + ~53 DNS, but the **branch hEX routers return IP+MAC with 0 names** (branch PCs have NetBIOS-over-TCP/IP off, and the DC has no reverse/PTR zones for `10.10.18x.x`), so branch static devices stay nameless via the network; a router-DNS change would NOT help (no PTR zones to resolve). Naming branch statics needs PTR zones on the DC, SNMP on the printers, or manual/AD naming. Prior work (2026-06-22/23): **UniFi controller as a 4th device source** (mig 053 — `node:https` login → `stat/sta` → merge into `dhcp_leases` by MAC `source='unifi'`; dedup synthetic IP-rows; `dynamic=NULL` not a false "Statická"); **long-term packet loss** (24h rolling window, mig 052 — `device_ping_samples`, online-only sampling); **IP revision by name** (DNS lookup persists current IP per AD machine); **scan site reconciliation** + `mikrotik-util.ts` extraction; **stale-lease pruning** (mig 054, `devices.lease_retention_days` default 14, non-destructive); **API connectivity panel** (`GET /integrations/status`, fast per-router `POST /mikrotik/test`); **Devices UX** (column header tooltips, fixed "Tiskárny" nav = Devices pre-filtered to printers, Stav tiskáren cards show hostname+note & filter by note); **report landscape A4**; **CSV semicolon+BOM** (Czech Excel columns+diacritics). Prior 2026-06-22 (LIVE `e3838a5`; full doc sweep): **shared/USB printers** (net view → Devices rows + Stav tiskáren section, located by host PC); **Devices tab UX** (editable note, row numbers, sortable headers, general search across ALL columns, "uncategorized only" filter, "Devices" dashboard tile = unidentified/total); **managerial report** (pie charts + summary incl. network-vs-USB printer counts + full list, A4, opened in a tab to dodge the HTTP insecure-download block); **device-scan reach** (nbtstat MAC fallback for remote subnets + Option B MAC-less-by-IP storage so live hosts always show; scan ranges 10.8.3/10.181.3/10.181.90 added); **per-category notebook eventlog noise suppression** (mig 051); **eventlog "PC v problémech" temporary per-PC snooze** with signature (mig 050). 102 tests. Draggable dashboard tiles were built then **reverted** per operator. See sessions below. Prior: 2026-06-17 (LIVE `e456fd0`; **full doc sweep done incl. G2** — README, ARCHITECTURE, dashboard.html CS+EN, project-status.html, i18n all current). **G2 — printer supplies + EWS proxy**: new "Stav tiskáren / Printer status" tab + 🖨 Náplně tile reading ink/toner/maintenance-box/drum/belt via **SNMP Printer-MIB primary + HTTP fallback** (Brother toner %, Epson maint box) with a self-contained `node:dgram` SNMP client; the device web-UI **cert-bypass proxy is now ON by default** and was reworked to actually render Epson/HP/Brother EWS (buffers body, `<base>`=doc dir, absolute-URL rewrite, relaxed CSP + MIME correction, client-side redirects); migrations 048–049; 74 tests. Earlier device-platform batch: MikroTik DHCP collection LIVE + fully DB-driven (no MIKROTIK_* env except MIKROTIK_SECRET); multi-source inventory merged by MAC = DHCP (dynamic+static reservations) + router ARP + active app-server subnet scan (configurable ranges CIDR/wildcard, `!` excludes a subnet, discovery cache, remote subnets via router ARP); NetBIOS (nbtstat) device names → printer auto-suggest (NPI/BRN/BRW/RNP/KMBT); Static/Dynamic Type column; operator-editable device name (mig 046); generic + configurable categories; per-device packet loss + latency (mig 045/047) as `ms/%` column with tunable problem thresholds, "Loss/latency" tile + "issues only" filter; printer-offline alert agenda (mig 043); 🖨 Printers tile; printer IP→web-UI with optional cert-bypass server proxy; new Database tab; deploy robocopy `/MIR` excludes `dist` (frontend-window fix). Migrations 043–047. — prior 2026-06-16: (decision: MikroTik DHCP collection simplified to IN-APP on the application server — external .225 sync-script model retired; pending one router allowed-address change · docs/i18n deployment-model corrected) — prior 2026-06-15: Ports availability tab + per-port latency · per-PC refresh now probes ports too · cmd-like ping console · Per-PC Actions trimmed to refresh-only · dashboard Ports tile + tile-click filter pre-select · Devices tab = MikroTik DHCP inventory paired with AD by hostname/IP · device categories by MAC + vendor suggestion · MikroTik config in Settings with AES-encrypted password · migrations 041–042

## Session 2026-08-03 — Přesná login history z eventlogu (4624/4634)

Kód na `main`, **zatím NENASAZENO** — čeká migrace 081 + restart služby na .213. Migrace **081**.

**Odkud to přišlo.** Otázka operátora: „bere se login history z eventlogu?" Ne — dosavadní „Historie přihlášení" je jen **WMI snímek** (`Win32_ComputerSystem.UserName` při každém kolečku disk-collectoru → `pc_user_history`), takže zachytí jen toho, kdo byl přihlášený v okamžiku scanu; krátká přihlášení mezi koly se ztratí a přesný čas login/logout tam není. Návrh: doplnit to o přesná data ze **Security logu** (4624/4625/4634). Zvolený rozsah (operátor): **přihlášení + odhlášení** (4624+4634), bez neúspěšných pokusů.

**Verify-core PŘED stavěním (na živém PC, svc-itdashboard → TRNKAMW11).** Potvrzeno: **Audit Logon = Success zapnutý** (4624 existují), účet **umí číst Security log na dálku** (Event Log Readers OK, nic v GPO na právech řešit netřeba), interaktivní přihlášení jsou přítomná. Zároveň to potvrdilo objemový problém: z 200 nejnovějších 4624 bylo **103 network (typ 3) + 95 service (typ 5) a jen 2 interaktivní** → ~99 % šum. Proto je to **samostatný collector, ne přepínač** u System/Application eventlog collectoru (ten navíc filtruje `Level 1-3`, kdežto přihlášení jsou `Level 4/Information`).

**Co se přidalo.**
- Migrace **081** `pc_logon_events` (computer_id, user_name, domain, logon_type, ip_address, `logon_id`=TargetLogonId hex, `logon_at`, `logoff_at` NULL dokud nedojde 4634, created_at) + kurzor `computers.last_logon_collected_at` (**oddělený** od `last_collected_at`, který vlastní eventlog collector) + `sp_purge_pc_logon_events` + settings `pcLogonHistory.retention_days`=90, `checks.run_logon`=1.
- `services/logon-collector.ts` — `Get-WinEvent -ComputerName -LogName Security -FilterXPath` s **filtrem typů přímo v XPath** (2/7/10/11), takže type-3/5 vůbec neopustí cílový stroj; z Event XML se tahá `TargetUserName/Domain/LogonType/IpAddress/TargetLogonId`; 4624 se insertuje (dedup na computer_id+logon_id+logon_at, protože kurzor `>=` může hraniční event přetáhnout), 4634 uzavře nejnovější otevřenou session se stejným LogonId. Strojové/systémové účty (`*$`, SYSTEM, DWM-/UMFD-…) se vyházejí. `tryWithHostLock(pc:<id>)` = busy PC se přeskočí a chytne příště. **Čistý add-on** — nesahá na `reachable`/`consecutive_failures`/`last_status`.
- Zapojení: nový check `logon` v `checks-runner.ts` (default on, běží hned po `eventlog`), krok 6 v `refresh-single-pc.ts` (ruční refresh jednoho PC), úklid v `retention-runner.ts` (+ `VALID_STEPS`).
- Endpoint `GET /computers/:id/logon-history?days=`; klient `api.logonHistory` + typ `PcLogonEventItem`.
- **UserHistoryModal** teď ukazuje dvě sekce: nahoře **„Přesná přihlášení (z eventlogu)"** (uživatel, typ=konzole/RDP/odemčení/cache, přihlášení, odhlášení / „probíhá", IP, trvání) a pod ní stávající **„Snímky přihlášení (odhad)"** WMI tabulku; když jsou přesná data prázdná, plain hláška „audit vypnutý / nikdo interaktivně" — graceful degradation, ne chyba (memory `outputs-legible-to-outsiders`).
- Settings: toggle `checks.run_logon` v periodických kontrolách + řádek v retenční tabulce. i18n CS+EN (`logonHistory.*`, `settings.check.logon`, `settings.ret.pclogon`, retenční step).

**Stav ověření / co dodělat při nasazení.** Verify-core **pro předpoklady** hotový; **přesný `FilterXPath` řetězec ještě NEBĚŽEL end-to-end** proti reálnému stroji (postaveno po schválení rozsahu). Při prvním nasazení ověřit na jednom PC z .213:
```powershell
$s = (Get-Date).AddDays(-3).ToUniversalTime().ToString('o')
$xp = "*[System[(EventID=4624 or EventID=4634) and TimeCreated[@SystemTime>='$s']] and (EventData[Data[@Name='LogonType']='2'] or EventData[Data[@Name='LogonType']='7'] or EventData[Data[@Name='LogonType']='10'] or EventData[Data[@Name='LogonType']='11'])]"
Get-WinEvent -ComputerName TRNKAMW11 -LogName Security -FilterXPath $xp -MaxEvents 20 | Select TimeCreated,Id | Format-Table
```
Vrátí-li 4624/4634 řádky → XPath v collectoru sedí. Chyba „Invalid query" → jednořádková oprava v `logon-collector.ts`.

**Nasazení (ručně na .213, mimo tuto session):** `npm run migrate -w @itdashboard/server` (aplikuje 081) → build → restart služby (`⟳ Restart služby` v Settings nebo `net stop/start ITDashboardAPI`). Kurzor cold-start = 24 h zpátky; první běh natáhne poslední den. **DB migrace + restart dělá operátor** (zápis do sdíleného SQL, klasicky mimo classifier).

## Session 2026-07-20 — Náhodná / lokálně spravovaná MAC (🎲) v inventáři

LIVE `2b558ee` (klient-only, nasazeno auto-deploy pipelinou — živá SHA `e9038c2`). **Bez migrace** (migrace zůstávají 001–080).

**Odkud to přišlo.** Otázka operátora: „Windows 11 má prý randomizované MAC jako standard — nevadí to inventáři klíčovanému MAC?" Přesná odpověď: randomizace se týká **jen Wi‑Fi** (drátový Ethernet MAC **nerandomizuje nikdy** → stolní domdoménová PC jsou v pohodě), a i na Wi‑Fi je adresa defaultně **stabilní per‑SSID** (ne mění se každé připojení). Reálně to kouše u **notebooků na Wi‑Fi** a hlavně u **BYOD mobilů** (Android 10+/iOS 14+ randomizují per‑SSID by default).

**Co se přidalo (čistě klient).** `isRandomMac(mac)` v `apps/desktop/src/api.ts` — **pure** funkce, testuje *locally‑administered* bit (`0x02` v prvním oktetu; druhá hexa číslice prvního oktetu ∈ {2,6,A,E} u unicastu). Odchytí Wi‑Fi randomizaci i virtuální adaptéry (Hyper‑V/VMware/docker); synthetic `IP-…` řádky a neúplné MAC → `false`. V matici zařízení (`DevicesPage.tsx`) se u takové MAC vedle adresy zobrazí **🎲** s nápovědou (`devices.randomMacTip`, CS+EN): není to HW adresa, výrobce z ní nepoznáme, není to trvalá identita, drátové PC se to netýká.

**Proč to nechce migraci ani zásah do serveru/collectoru.** Randomizovaná MAC má nastavený lokální bit, takže **nikdy netrefí reálné (universálně spravované) OUI** — ani `PRINTER_OUI`, ani `svcports.voip_ouis` → `suggestCategory` i párování na AD se chovají správně **samy**, bez úprav. Sedí to s modelem autority (drát/DHCP = 1. autorita pro IP/přítomnost s reálnou MAC, AD = 2. pro identitu). Manažerský souhrn už dřív telefony (randomizovaná MAC) z „techniky" vylučoval (`equip = category !== 'phone'`) — to zůstává beze změny.

**Ověření (verify-core dřív než stavět okolo).** `vitest` — 6 nových testů proti reálným MAC (`94:DD:F8…`/`00:07:4D…`/`AC:3F:A4…` HW → ne; `DA:A1:19…`/`A6…`/`02:…`/`7e-45-…` → ano; synthetic/partial/empty → ne), `api.test.ts` **45/45**; `tsc --noEmit` čistý; `vite build` OK. Vizuál v živé appce nespuštěn (chce běžící server+DB); render badge je triviální podmínka nad otestovanou logikou.

**Dokumentace.** Full sweep: README (Devices bullet + Status), ARCHITECTURE (device-discovery sekce), `docs/nacitani-mac-hostname.html` (nová sekce 5 — MAC identita vs. randomizace), `docs/project-status.html` (Poslední změny), obě prezentace `docs/prezentace-system.html` + `presentation-system.html` a in-app `PresentationPage.tsx` (poznámka u „Identita = MAC").

## Session 2026-07-16 — Disabled Office add-ins (Navision export returning nothing)

LIVE `ba999ab`. Migrations **001–080**.

**A) Where it came from.** "Export from Navision to Excel used to build a table, now it does nothing — a colleague on the same Microsoft 365 is fine." Diagnosis on the reporting workstation, in order: NAV *does* produce files (six `.xltx` in `%TEMP%`, one per attempt) → the `.xltx` is **intentionally empty**, no `sharedStrings.xml`, only `CustomProperty*.bin` carrying `Server=navision`, port `7101`, `ServerInstance=NAV-LIVE` → so the data is supposed to arrive via the **NAV Excel add-in over OData**, not from NAV itself → the add-in was sitting in `HKCU\...\Office\16.0\Excel\Resiliency\DisabledItems`. Office had disabled it after a crash and said nothing, anywhere. Removing that value fixed it.

**Dead ends worth not repeating** (all checked and all fine): Office bitness (64-bit Office against the 32-bit RTC is supported; `Microsoft.Dynamics.Nav.ExcelAddIn.dll` is AnyCPU), the `.xltx` file association (correctly points at EXCEL.EXE), `LoadBehavior` (=3, HKLM), Office PIA presence, and the Event Log (**nothing** — Office does not log a disable). The only NAV client crash in the log was an unrelated `NullReferenceException` in `NavLookupAction`.

**B) Why it generalised.** The same single registry read finds **any** disabled Office add-in, not just NAV — the reporting workstation also had an M-Files add-in registered. So the feature is "disabled Office add-ins", with NAV called out separately because that is the one with a known, invisible business consequence.

**C) Transport, and the constraint that shaped the design.** `DisabledItems` is per-user (HKCU). Remotely that means **HKEY_USERS** (hDefKey `2147483651`) via **StdRegProv in a DCOM CimSession** — the same shape as `services-collector.ts`; plain `Get-CimInstance` would need WinRM, which domain PCs don't have. Two things were verified before building rather than assumed:
- **`NTUSER.DAT` is exclusively locked while the user is logged on** — so the offline `C$` route used by crash-dump collection cannot work here. Confirmed by trying to open it.
- **`HKEY_USERS` only contains hives of logged-on users** — so a powered-on but logged-off PC yields nothing.
- `RemoteRegistry` is `Stopped / Automatic (trigger-start)` — it starts itself on connect, nothing to enable.
- Rights: `svc-itdashboard` ∈ `Server Admins` ∈ local Administrators on the client → satisfies the `winreg` ACL. (An initial test failed only because it ran as an unelevated interactive user, which says nothing about the service identity.)

Consequence baked into the model: `office_addin_scans.status` is `ok` | `no_users` | `error`, and **`no_users` is not `ok`**. The Computers column renders `—` / `· nikdo` / `✗` / `●` / `⚠ N` as five distinct things, and the tile counts only `status='ok'` PCs, pushing unknowns into a badge. Same tri-state discipline as `computers.reachable`.

**D) Verified — and the first run immediately proved the decode wrong twice (migration 079).** `POST /office-addins/scan` with the feature still disabled: `pcs=93 scanned=65 withIssues=22 navDisabled=0`. The DCOM/HKU read itself works. The *data* exposed two mistakes that only real fleet output could show:

1. **`DisabledItems` holds disabled DOCUMENTS too, not just add-ins.** Real finds: `c:\mail\jaksikovav.pdf`, `c:\users\<u>\...\content.mso\e96c092a.doc`. Office records a file that crashed it. Counting those as "disabled add-in" inflated the numbers — a PDF that once upset Word is not a silently broken application. Now classified by path extension (verifiable) into `item_kind` = `addin` | `document`; **only `addin` feeds `disabled_count`, the tile, the column and the manager KPI**. `raw_type` (the header DWORD) is stored but not trusted — its meaning is only known for `type=1`.
2. **The regex scrape glued header garbage onto names**, e.g. `愀蓶ｼ侴산촂뙿﹨Prohlížeč náhledů: Náhled Microsoft Wordu` — header bytes decoded as CJK. The value has a **fixed structure**, confirmed to the byte on a real capture: `0x00 DWORD type · 0x04 DWORD cbPath · 0x08 DWORD cbName · 0x0C UTF-16LE path · UTF-16LE name`, with `12 + cbPath + cbName == value size` (250 + 68 + 12 = 330). Now parsed by those length prefixes; a value whose lengths don't fit is dropped rather than guessed at.

Genuine finds after the fix are real add-ins: Word/Excel preview handlers, `teamsaddin.fastconnect`, Brother printer add-ins, `m-files office add-in`, `pdfmoutlook.dll`. Split by app: Word 25, Outlook 7, Excel 4. `navDisabled=0` — the reporting workstation was the only NAV case and was already fixed. 19 PCs returned `no_users`, confirming the logged-on-only limit is an everyday condition, not a theoretical caveat. The only `error` results were the **domain controllers** (`B-S-W-DC-01/02/03`) — the service account is not admin there, the same expected failure the disk/link-speed collectors document.

**Lesson worth keeping:** the unit tests and the parser check passed the whole time. Neither could see this — the decode was *self-consistently wrong*. Only real fleet data showed it.

**E) Clean numbers after the rewrite + its own tab.** Rescan on `28b576e`: `pcs=86 scanned=57 withIssues=7 navDisabled=0`; the 079 backfill dropped affected PCs from the inflated **22 to 8** (of 36 stored rows, **25 were documents, 11 add-ins**). Real disabled add-ins are **Outlook 7** (`teamsaddin.fastconnect` x4, `adobeacrooutlook.sendaslink` x2, `pdfmoutlook.dll`) and **Word 4** (`m-files office add-in`) — **Excel 0**, the reporting workstation was the only Excel/NAV case and was already fixed. So the live payoff is not NAV at all: people have **Teams silently broken in Outlook and M-Files in Word** and nobody knew.

That also killed the original UI plan. A count in a Computers column answers "how many" but not "which add-in, in which app" — and with 11 findings across 8 PCs, "Office" as a label is useless to an operator. Added a dedicated **🧩 Doplňky** tab (`OfficeAddinsPage.tsx`, nav next to 💥 Pády): one row per finding with app (icon + version), add-in name, PC (jumps to Computers), user, path, scan age; filters by app and search; documents hidden behind a checkbox and dimmed when shown; NAV pinned to the top; `⟳ Skenovat` triggers the sweep; a banner states plainly when the scheduler is off, so nobody reads stale data as live. The homepage tile now opens **this** tab, not the Computers list. The Computers column stays as the per-PC signal and its tooltip points at the tab. 19 came back `no_users` and the 3 DCs came back `error` (service account is not admin there — the same expected failure the disk/link-speed collectors document). The escaping is parser-checked plus unit-tested (TS template literal → PS → regex is silent when wrong: PowerShell doesn't treat `\` as an escape, so a malformed path never throws — it just never matches, and every PC would report "clean"). The binary decode **is** verified against a real captured `DisabledItems` value: 330 bytes → correct .dll path, add-in name, and NAV detection. First run with `officeaddins.enabled=1` is the remaining test.
**F) Email alerting — its own agenda, its own cadence (migration 080).** Without a mail nobody acts; the data would sit in the dashboard until somebody thought to look, which is the exact failure mode the feature exists to fix. Same shape as disks/services/ports/printers: `alerts.officeaddins.enabled` (off), `.recipients`, `.frequency_hours`, `.debounce_minutes`, `.maintenance_window`, state in `office_addin_alert_state`, evaluated at the end of each scan, `POST /alerts/officeaddins/test` for an SMTP check from Settings.

Two deliberate departures, both asked for by the operator ("agendu bude mít třeba zase někdo jiný"):
- **Own recipient list.** Office add-ins are usually a different person's agenda than disks — helpdesk / application owner, not the server admin. Empty falls back to the shared `alerts.recipients`, so single-list installs are unaffected.
- **Throttle = 1 WEEK (168 h), debounce = 0.** A disabled add-in is a **state, not an outage**. Nobody fixes it within the hour, so daily nagging would turn these mails into noise people stop reading. And there is no flapping to debounce — the scan runs every 6 h and the finding is either in the registry or it isn't.

Alert key is `computer|app|addin` (lowercased), **not** `value_name` (a hash that need not survive an add-in reinstall). Re-enabling an add-in drops its state row, so a relapse alerts again instead of being swallowed by a stale `last_sent_at`. Only `item_kind='addin'` is alerted — mailing about a PDF that once crashed Word would be noise.
## Session 2026-07-01 — Link-speed / connection-quality measurement

LIVE `d7d6890`. Migrations **071–074** (`link_speed_results` + `latency_ms` + `cycles` +
`linkspeed.*` settings). A "Měření linky" (⚡) tab + homepage tile.

**Core** (`services/link-speed.ts`): for each target, `ensureSource()` caches an N-MB random
file on .213, then per cycle it `pipeline`s it to `\\PC\C$\tmp\itdash-speedtest\<filename>` (upload)
and back to .213 (download), timing each; keeps the **best** up/down of `cycles` cycles; pings for
RTT. **The whole client dir is removed afterwards** (success and error). Real endpoints (not a
router CPU), over the admin-share `C$` the service account already uses — so it's the operator's old
PowerShell speed-test, in-app, and it exposes **bad cables / 100-Mb ports** on the 1 Gb fleet.

**Batch + page** (`LinkSpeedPage.tsx`, `POST /system/linkspeed/run`): targets = IP list / `10.8.2.*`
/ `10.8.2.180-182` / `all` (→ `expandTargets()` resolves active PC/server IPs, drops
`linkspeed.exclude_hosts`). Sequential background run, **SMB-prechecked (445)** so dead IPs skip
fast — `offline` (no ping, dim) vs `SMB/445 blokováno` vs `EPERM` (DCs). `■ Přerušit`
(`POST /system/linkspeed/stop`). Live **cmd-like console** of the batch + **activity-log** lines
(source `linkspeed`). Sortable grid, **per-column operator filters** (`>N`,`<N`,`a..b`, date ranges)
+ global search + status filter; IP→Computers link, Hostname/Latency/Cycles columns; CSV/HTML/PDF
export of the view. **Overview** visual (chips + slowest-link bars). **⚡ Linky homepage tile**
(`GET /system/linkspeed/summary`, latest-per-target). **Settings** "Měření linky (plán)": scheduler
(enable / interval-h / **hour window** / targets / **hostname exclusions** / cycles / **filename** /
size / OK threshold). Scheduler is DB-driven + restart-safe and **rolling** (see 2026-07-01 rolling
scheduler session below): every `linkspeed.tick_min` tick it measures the stalest-N due PCs
(`linkspeed.batch_size`) whose last measurement is older than `linkspeed.interval_hours` (now a
per-PC refresh window, not a sweep period); freshness is read from the DB so it survives restarts,
only within the hour window.

**Ops notes.** The `linkspeed.filename` (default `itdash-speedtest.tmp`) + path
`C:\tmp\itdash-speedtest` should go into **AV (ESET) exclusions** — no extension/archive dodges
on-access scanning (ESET reads content); a path exclusion is the only real fix. Servers/DCs error
with `EPERM` (svc-itdashboard isn't a Domain Admin). Data cost: size × PCs × 2 × cycles — mind it
for `all`. Verify-core was done live (20 MB to TRNKAMW11) before building.

## Session 2026-07-01 (pokračování) — Link-speed: metody měření, běhy, sítě, per-host lock

LIVE `848da26`. Migrations **075–077** (`ip_address`+`host_name`; `nic_mbps`+`nic_name`+
`robo_up_mbps`+`robo_down_mbps`; `run_id`). Builds on the same-day link-speed module (above);
all nasazené a živé. UI strings CS (+EN kde relevantní).

**A) Měření linky — UI/ovládání.**
- **Pole "Cyklů"** na stránce: prázdné = hodnota z Nastavení, vyplněné `1–20` má přednost. Platí
  pro **dávku i per-PC test**.
- **Zaškrtávátko "Ignorovat výjimky z Nastavení"** — jen u **RUČNÍHO** běhu; plánovaný běh
  respektuje `linkspeed.exclude_hosts` vždy.
- **Výběr běhu**: Poslední běh / Poslední 3 / Poslední 10 / Všechny běhy. **Každá dávka = jeden běh**
  s otiskem **`run_id`** (= čas startu dávky, mig 077). Souhrn, panel sítí i tabulka se řídí výběrem.
- **↺ Reset souhrnu = NEDESTRUKTIVNÍ**: nastaví baseline (`linkspeed.baseline_at`), **NEMAŽE z SQL** —
  historie zůstává; souhrn/verdikt se jen počítá od baseline dál.
- **Terminál**: u IP i hostname + stav cyklů živě ("cyklus k/N", u hotových "N×").
- **Sloupec KDY** kompaktní `DD.MM.RRRR HH:MM:SS`; **filtr data parsuje CZ i ISO** (`>`, `<`,
  rozsah `a..b`).
- **Tooltipy (`title`)** na všech hlavičkách historie.

**B) Metody měření** (Nastavení → "Metody měření", každá vypínatelná; settings
`linkspeed.method.smb/robocopy/nic`).
- **SMB kopie** (default **ON**) — single-stream ↑↓. **VERDIKT se počítá JEN z tohoto** (to uživatel
  cítí u jednoho souboru).
- **Robocopy /MT** (default **OFF**) — multi-file paralelní. **DŮLEŽITÉ:** read-back bývá
  **cache-inflated** (ukáže i > rychlost portu, např. **2800 Mb/s na 1Gb portu**) → **NEDŮVĚRYHODNÉ**;
  sloupec **RC je jen informativní, NErozhoduje verdikt**.
- **NIC rychlost portu** (default **ON**) — vyjednaná link speed přes **DCOM CimSession** (stejná
  cesta jako disk kolektor, **NE WinRM**); sloupec **Port**; `<1000` = 100Mb port; nestandardní rate
  nebo "wireless/Wi-Fi" v názvu adaptéru = **wifi**.
- **Prodleva mezi PC** (`linkspeed.pause_ms`, 0–60000 ms) mezi cíli v dávce.

**C) Vyhodnocení / výstup.**
- **IP i hostname VŽDY na výstupu** (mig 075): hostname cíl → **forward DNS na aktuální IP** (pozná
  wifi vs kabel), IP cíl → jméno z inventáře.
- **Wifi = vlastní kategorie**: řádek modrý štítek "wifi", **NEsoudí se drátěným prahem**; wifi je
  v počtech/sítích/reportu; alarm "100Mb port" wifi **vylučuje**.
- **Panel "Sítě (/24)"**: verdikt **problematická síť** (systémová — pomalá většina, ≥2 měřené) /
  **jednotlivé PC** / **wifi** / **"0 měřeno"** (síť existuje v inventáři zařízení, ale nezměřená —
  červeně šrafovaně). **Klik na síť předvyplní Cíle.** Filtruje balast APIPA `169.254.*`, `127.*`,
  `0.*`.
- **HTML report** (🔗 otevře **NOVÝ LIST**, ne download): "Co kontrolovat" (problematické sítě / PC
  na 100Mb portu / nejpomalejší), počty (vč. wifi), vizuál sítí jako pruhy (vč. 0 měřeno/wifi),
  detailní tabulka **ŘAZENÁ PODLE IP**. Export CSV/HTML/PDF.

**D) Zdroj cílů `all`** = **počítače z AD** (tabulka `computers`), `WHERE enabled=1 AND excluded=0
AND ip_address NOT NULL AND (reachable=1 OR (reachable IS NULL AND consecutive_failures<10))`.
**NE** z DHCP/UniFi inventáře. (Dřív tvrdé `reachable=1` → uvolněno.) Výjimky berou
hostname/IP/rozsah/wildcard.

**E) Per-host zámek** (nový princip, `apps/server/src/services/host-lock.ts`). Těžké per-PC operace
(SMB C$, WMI, robocopy) jsou **serializované na IDENTITU počítače** (`pc:<id>`, **ne IP** — ta je
dynamická). **`withHostLock`** = blokující (link-speed), **`tryWithHostLock`** = přeskočí zaneprázdněné
PC (crash-dump). **Různé PC běží paralelně.**

**F) Restart služby.** Nastavení → tlačítko **⟳ Restart služby** (vedle Uložit).
`POST /system/service/restart` spustí **ODPOJENÝ proces** (start mimo NSSM strom) →
`net stop/start ITDashboardAPI`. **Ověřeno živě** (služba naběhla sama).

**G) Migrace / nastavení / endpointy.** Migrace **075** (`ip_address`,`host_name`), **076**
(`nic_mbps`,`nic_name`,`robo_up_mbps`,`robo_down_mbps`), **077** (`run_id`). Nová nastavení
`linkspeed.method.smb/robocopy/nic`, `linkspeed.pause_ms`, `linkspeed.baseline_at`. Nové endpointy
`/system/linkspeed/reset`, `/system/service/restart`; `/linkspeed/run` bere `cycles` +
`ignoreExclusions`.

## Session 2026-06-30 (late) — Service-port matrix + discovery (BUILT, then PARKED)

LIVE `d55552d`. Migrations **065–070** (settings seeds only; no new tables). Feature is
**parked**: nav tab hidden, `svcports.enabled=0`. All code/endpoints stay.

**Goal:** "is the same service reachable the same way on every branch?" (printers,
IP phones, …) — a per-branch consistency matrix.

**1) Matrix** (`services/service-port-matrix.ts`, scheduled `startServicePortsSchedule`,
`svcports.interval_sec`=900). For each `svcports.checks` entry `Label:port:category`, probes
that TCP port (reusing `tcpProbeTimed`, now exported from the port-status collector) on every
categorized device of that category at each site (`mikrotik.routers`), **from .213 over the
WAN**. Each cell = `open / online` plus **offline counted separately** (an offline box isn't a
blocked port — so a site whose devices are all offline shows "N off", not an empty "—").
**OUI-aware:** `svcports.voip_ouis` (default `805ec0,249ad8,001565`) promotes a Yealink/VoIP-OUI
device to an effective category `voip` regardless of its stored category. `GET /system/service-ports`;
`ServicePortsMatrix.tsx` renders the matrix and, on a **cell click, lists the actual devices**
(open/closed/offline, IP+name).

**2) Discovery scan** (`services/service-discovery.ts`, `POST /system/service-discovery`,
**user-triggered** — a deliberate probe sweep). Scans a broad TCP set (1–1024 + common highs, or
the full 1–65535 with `{full:true}`) on a **sample of reachable devices per category**
(`svcdisc.sample`/`full_sample`), reports per category which ports are open and on how many
devices — so the monitored check list is grounded in what's actually running. VoIP split out by OUI.

**3) Migrations 065–070:** 065 `service_port_matrix` (svcports.enabled/interval/timeout/max_per_cell/
checks), 066 drop the bogus `phone` SIP default (it probed guest-WiFi mobiles), 067 `service_discovery`
(svcdisc.* + svcports.voip_ouis), 068 add a `voip` web-80 check, 069 drop it again, 070 **park**
(`svcports.enabled=0`).

**Why parked (the honest arc).** The matrix probes from the **central .213**, so it measures "can
the **CENTRE** reach device X", not "can the **BRANCH** reach it". The operator's real need is
**"do the branches print to the central office?"** (confirmed: yes they do) — which is a property of
the **branch→HQ path** and can only be measured **from the branch router**, not from .213
(same central-vantage ceiling as the WAN speed test and VoIP). Cross-site rows (HQ `10.8.2.x` IPs
appearing under a branch) are **expected, not dirty data** — branch routers route into HQ so users
can print there. And the org is **mid SD-WAN redesign separating the printer network into its own
segment**, so the right target is a **printer-segment subnet that doesn't exist yet** (moving
target). Decision: **park** until the SD-WAN segment lands, then revive as **per-branch reachability
to the printer segment, measured from the branch routers** (MikroTik `netwatch type=tcp` or
`/rest/ping`). Caveat for revival: the `dhcp-reader` API user is **read-only** — running a router
test / creating netwatch will likely need a higher policy on the routers (just as `/tool/fetch`
only worked as `admin`).

**VoIP finding (out of scope from .213).** The IP phones are **Yealink** (OUI 24:9A:D8, e.g. MAC
`249AD8C45023`) registered to a **T-Mobile cloud PBX** (BroadWorks, portal `xspweb.t-mobile.cz`).
Verified live: **zero** Yealink-OUI devices are in `dhcp_leases` — the phones sit on a voice
network the polled routers don't inventory, so `.213` can't see or reach them. SIP is UDP to the
cloud anyway. So VoIP monitoring from ITDashboard is **deferred**; the authoritative "is the phone
registered / can it call" status lives in the **T-Mobile portal / BroadWorks XSI/OCI API**, not here.
The matrix is OUI-ready for the day the phones become visible (voice subnet added to
`mikrotik.scan_ranges` AND reachable from .213). Memory: `itdashboard-voip-out-of-reach`.

**Also this session (already in the 2026-06-30 doc sweep):** the WAN speed test now uses **parallel
streams** + first-byte timing (a single stream read 167 Mb/s on a link fast.com saw at 420; parallel
now reads ~370). Note on data cost: `wan.speedtest_url` default `bytes=25000000` = **25 MB/stream**,
× `wan.speedtest_streams`=6 = **~150 MB/test**, × 48/day at the 30-min cadence ≈ **~7.2 GB/day**
from .213 — tune bytes/streams/interval down in Settings if that's too much.

## Session 2026-06-30 — Komunikace tile, WAN monitor + speedtest, Pády status & report

LIVE `e0db733`. Migrations **062–064** (WAN monitor + speedtest settings; no new tables — WAN
state is an in-memory snapshot). All UI strings bilingual (CS+EN).

**1) Dashboard "Komunikace" health tile + `GET /system/comms`** (`routes/system.ts` — new file).
Aggregates **6 communication channels** into one `overall` (`ok`/`degraded`/`down`):
`database` (live `SELECT 1` + latency), `mikrotik_rest`, `ftp` (per-site freshness from
`site_data_status`), `collector` (EventLog DCOM to client PCs), `email` (SMTP/M365 — alert sends
log under `activity_log` source `alerts`, sent=`warn`/fail=`error`), `unifi`. **Rule:** a channel
is OK unless its **last** `activity_log` entry is `error` (per-PC collector `warn` doesn't flip it);
FTP also fails on stale/missing per-site data; unconfigured channels = `enabled:false` "vypnuto"
and are excluded from the count. Frontend: **📶 Komunikace** tile (`SummaryCards`) — `✓` when all OK,
else `okCount/total`, green/amber/red — opens a collapsible **`CommsHealth`** panel (one row per
channel: status dot, detail, last error, last-OK time). Fetched on mount + the 30 s dashboard loop.

**2) WAN monitor** (`services/wan-monitor.ts` — new, `startWanMonitorSchedule()` in `index.ts`,
DB-driven enable/interval like the crash collector). Every `wan.interval_sec` (default 60) it pings
**from .213** each branch router IP (parsed from `mikrotik.routers`) **and** one internet target
(`wan.internet_target`=1.1.1.1) using Windows `ping.exe` with the locale-independent `TTL=`/`[<=]NNms`
parse (copied from the mikrotik collector) → **avg RTT + packet loss %**. Keeps only the **current
snapshot in memory** (the operator wants the live state, not history). `GET /system/wan` returns the
branches, the internet probe, thresholds (`wan.latency_warn_ms`=80 / `wan.loss_warn_pct`=5),
`nextRunAt`, and the optional speed. UI: always-visible **`WanHealth`** strip on the dashboard
("📡 Komunikace na pobočky a internet") — one chip per site with a green/amber/red left border and
`RTT ms · loss%`. Live numbers proven: Zastavka ~59 ms, Svitavy ~13 ms, Jihlava ~20 ms, internet
1.1.1.1 ~10 ms, 0 % loss.

**2b) Central download speed test** (opt-in, **OFF by default**). When `wan.speedtest_enabled`=1 the
monitor, on its own longer cadence (`wan.speedtest_interval_sec`=1800), downloads `wan.speedtest_url`
(default Cloudflare `https://speed.cloudflare.com/__down?bytes=25000000`, no token, HTTPS) over
**`wan.speedtest_streams`=6 PARALLEL streams** (a single stream can't saturate a fast link — TCP
window/latency cap it), sums the bytes and times **from the first byte received** (excludes
DNS/TCP/TLS/TTFB). Shown as "↓ X Mb/s" on the Internet chip. **Why parallel:** a single 10 MB stream
read 167 Mb/s on a link fast.com measured at 420; parallel + first-byte timing now reads ~370 Mb/s.
Measures the internet **from the server**, not per-branch. Settings section "Komunikace na pobočky a
internet" exposes the whole monitor (enable, ping interval, internet target, both thresholds, speed
enable/URL/interval/streams). Migrations: **062** `wan_monitor` (enabled/interval/target/ping_count/
thresholds), **063** `wan_speedtest` (enabled=0 / url / interval), **064** `wan_speedtest_streams`
(streams=6 + bump default URL to 25 MB for untouched installs).

**2c) Per-branch speed = dead end (verified live, don't retry).** "Involve the MikroTik" via
`/tool/fetch` does NOT measure the branch's internet — RouterOS fetch is single-threaded and
**CPU/TLS-bound on a hEX**: `:put [/tool fetch url="…__down?bytes=25000000" mode=https output=none
as-value]` on Zastavka returned `downloaded=24414 KiB; duration=00:00:51` = **~3.9 Mbps**. So
per-branch throughput was dropped; branches show latency+loss only. Real branch capacity needs a
download from a real PC at the branch (would hang off the existing client collection). Memory:
`itdashboard-wan-monitor`.

**3) Pády (crash) page — status strip + report overhaul + smarter explanation.**
- **`GET /crashes/status`**: `crash-dump-collector.ts` now tracks `lastRunAt` / `nextRunAt` /
  `lastResult` (`getCrashCollectStatus()`); `nextRunAt` is set by the scheduler from the **live**
  `crash.interval_sec`, so it follows Settings. The route adds `MAX(ingested_at)` (last SQL write),
  `MAX(analyzed_at)`, `enabled`, `intervalSec`, `pending`. `CrashesPage` shows a strip above the
  aggregates: **🛰 Poslední sběr** (+relative + PC ratio) · **💾 Poslední zápis do SQL** ·
  **⏭ Další sběr** (relative, or "sběr vypnutý"). i18n `crash.coll.*`.
- **Report = printable + dark/light.** The cdb `<pre>` is now **light by default** (printable; the
  dark block used to print as light-on-white = unreadable). A standalone **◐ light/dark toggle**
  button (`html[data-dark]` attr) is injected into the saved/printed HTML; `@media print` hides it
  and forces light. The saved `.html` now gets a **UTF-8 BOM** so it opens from disk without
  `AnalÃ½za pÃ¡du` mojibake.
- **Explanation derived from the dump** (`buildExplanation()` in `CrashesPage.tsx`), shown in the
  report **and** the live detail panel: "Co se stalo" (plain), "Technický rozbor", "Doporučený
  postup". `KERNEL_MODS` distinguishes a Windows-kernel (`nt`/`hal`/`win32k`…) fault from a named
  third-party driver. **Key fix:** `CORRUPTION_STOPS` (0x13A/0x19/0xC2/0x139/0x1A) — for these the
  fault is *detected* later by innocent code, so an all-`nt` stack does **not** mean "it's Windows";
  it means a driver (or bad RAM) corrupted memory and the culprit already left → recommends
  **Driver Verifier + memtest**. (Validated on SKALAMW11N 0x13A `KERNEL_MODE_HEAP_CORRUPTION` and
  ZAST23W10 0x133.)

## Session 2026-06-29 — BSOD / crash-dump collection + analysis

LIVE `4d8ef04`. Migration **061** (`crash_dumps`). **OFF by default** (`crash.enabled=0`).
Built after a full live verify-core (see "why it works" below).

**Goal:** turn "PC crashed" into "PC crashed because X" across the fleet, agentless,
from the data ITDashboard already has access to.

**1) Collector** (`crash-dump-collector.ts`, own self-rescheduling timer, `crash.interval_sec`
default 3600). Lists `\\PC\C$\Windows\Minidump\*.dmp` on monitored+reachable PCs
(TCP/445 pre-flight to fail fast), **dedup by `(computer_id, source_filename)`** so an
on-client dump isn't re-ingested every cycle, reads each NEW `.dmp` straight into a buffer
and INSERTs it as a blob with `status='pending'`. Never touches `C:\Windows\MEMORY.DMP`
(only the small minidumps). The on-client file is left for Windows to clean — the DB is the
durable store. Caps absurd sizes (>64 MB) out.

**2) Analyzer worker** (`crash-analyzer-worker.ts`, separate timer, `crash.analyzer_interval_sec`
default 300). Picks `pending` rows, **materializes the blob to a temp file on .213**, runs cdb,
parses, UPDATEs the row, **deletes the temp**. Decoupled from collection so a slow cdb never
stalls the fleet scan. The `crash_dumps.status` column IS the queue.

**3) cdb analysis** (`crash-analyze.ts`). **Important gotcha:** the newest cdb (10.0.28000,
Debugging Tools installed via Win11 SDK on .213) has a **broken headless `!analyze`**
(Extensions-Gallery async — emits nothing via `-c`/stdin/`-logo`). We do NOT use `!analyze`.
Sync commands DO work and are enough:
```
...
```
The parser pulls: STOP code (`.bugcheck`), bug-check name (lookup table), hot function
(`!dpcwatchdog` "functions that exist often"), offending **process** (`!thread`/`!process`
→ `Image:`), and the first non-nt/hal module in the stack. Proven live on ZAST23W10's real
0x133: `msedgewebview2.exe` closing a large memory section → `nt!MiDeleteSubsectionPages`
97/97 → no third-party driver. (For watchdog-type bugchecks the immediate stack is only the
interrupted context; the sync recipe still names process+hot-fn.)

**4) Routes** (`routes/crashes.ts`): `GET /crashes` (list, no blob), `GET /crashes/:id`
(detail incl. full `analyze_text`), `GET /crashes/:id/dmp` (download blob), `POST /crashes/run`
(on-demand collect+analyze, NOT gated by `crash.enabled` — for manual test).

**5) UI** (`CrashesPage.tsx`): 💥 **Pády** tab — aggregations (STOP breakdown / top culprit /
repeat offenders), table (PC · time · STOP · bug-check · culprit · status), per-crash detail
(STOP/bug-check/process/module/hot-fn + full cdb output + ⬇ `.dmp`), and a **white printable
"Analýza pádu systému" report** (🖨 print + ⬇ save HTML, same serializer approach as the
Manager Summary / Presentation). Plus a **💥 dashboard tile** (`crashStats` → tab) and a
**Settings → "Sběr pádů (BSOD)"** section (enable / collect+analyzer intervals / cdb path /
symbol path / blob retention). All bilingual (CS+EN).

**6) Permissions / deps (rebuild notes).** Needs **Debugging Tools for Windows** (`cdb.exe`) on
.213 — install via Win11 SDK: `winsdksetup.exe /features OptionId.WindowsDesktopDebuggers /quiet`
(.213 has no winget). Needs the service account `svc-itdashboard` to read client `C$` — it
already has it via the **Server Admins** AD group (member of clients' local Administrators).
`crash.cdb_path` / `crash.symbol_path` settings make both configurable.

**Why it works (verify-core, all done live before building):** transport (svc account reads
`\\PC\C$\Windows\Minidump`), blob write to `ITDashboard` on `B-S-W-SQL-04`, blob read-back by a
2nd connection (= worker) + analyze giving identical result, and full culprit attribution from a
minidump via the sync recipe. See memory `itdashboard-bsod-dump-collection`.

## Session 2026-06-26 (evening) — customizable dashboard tile layout

LIVE `2f74eac`. No migration (uses two new settings keys). All in the homepage
`SummaryCards` component + the two folded-in panels.

**Goal:** the homepage was cluttered and fixed. The operator can now lay the tiles
out themselves. An **✏️ toggle** (top-right of the cards) enters edit mode; the normal
view stays clean (no per-tile chrome).

**1) Coordinate placement.** Each tile's ✏️ opens a small editor: **position** =
column letter (A–H) + row number, e.g. `B3` (empty = auto-flow into the next free
cell); **size** = `1` (full) or `B` (half height + smaller font, sub hidden). Pinned
tiles hold their cell, the rest auto-flow. Editing **freezes** the full grid first so
moving one tile never reshuffles the others.

**2) Stacking — two halves per cell.** A cell holds ONE full tile OR **two half tiles
stacked**, addressed by an **upper / lower slot** (`B3` upper + `B3` lower). Two halves
(38px + gap) occupy exactly one full row (84px). Capacity is enforced — no two tiles on
the same slot, no half over a full.

**3) Per-row headings.** Each tile row can carry an optional heading (e.g. "Disky",
"Síť"), edited inline above the row in edit mode, shown as a section label otherwise.

**4) Persistence.** Layout → `dashboard.tile_layout`, headings → `dashboard.row_titles`
(both via the generic `PUT /settings`). A `useEffect` re-syncs from the loaded value
because settings arrive async (the first mount read empty → a refresh used to reset).

**5) Folded-in tiles.** "PC v problémech" (HealthCards) and "Operační systémy"
(OsBreakdownChart) were lone cards outside the grid; they are now real grid tiles via
`problemPcs` / `osBreakdown` props on `SummaryCards`, and each component renders ONLY
its detail panel (controlled `open` + `hideSummary`), toggled from the tile. The panels
got `flex-shrink:0` + body `flex:none` so they size to content instead of collapsing in
the flex-column dashboard view. The render was restructured into **row blocks** (heading
+ grid of cells), which also removed the empty-row gap.

**Note:** drag-and-drop tiles were tried long ago and reverted; this coordinate model is
deterministic, which is why it sticks.

## Session 2026-06-26 (late 2) — per-row retention run + Monitor/Exclude mutual exclusion

**1) Retention policy — per-row manual run.** The "Retenční politika" overview table
(`SettingsPage.tsx`, `#sec-retention`) gained a 4th column **"Ruční spuštění / Manual run"**:
every row now has its own checkbox + **▶ run button** (runs just that step), and shows the last
run's result inline (✓ rows-deleted / ✗). A toolbar under the table adds **Označit vše / Odznačit
vše / ▶ Spustit označené (n)**. The old separate checkbox block (`RetentionRunBlock`) is replaced
by a shared `useRetentionRun()` hook + presentational `RetentionReportView`; `ALL_STEPS` is now
derived from `RETENTION_ROWS` (each row carries a `step`). **All 9 rows are now runnable** — the
3 device-inventory tables (`dhcp_leases` ghosts, `device_ip_history`, `device_ping_samples`),
which were previously only pruned inline by the MikroTik collector, got **discrete retention
steps** (`dhcp_leases_purge` / `device_ip_history_purge` / `ping_samples_purge`) in
`retention-runner.ts` that replicate the collector's exact DELETE queries (`callQuery` helper;
ghosts honour "0 = keep forever"; loss-window always prunes with the +1h margin). `VALID_STEPS`
in `routes/retention.ts` extended to match. No migration — uses existing settings/tables.

**2) Computers — Monitor and Exclude are mutually exclusive.** A PC can be MONITOR **or** EXCLUDE,
never both checked. Enforced at the source of truth (`routes/computers.ts`): excluding a PC clears
`monitor_enabled`; enabling monitoring clears `excluded` — applied to both per-row PATCH endpoints
**and** the bulk endpoints (`/bulk-flag`, `/monitor/bulk`). The Computers UI refreshes from the
server after each toggle, so the state reflects immediately; checkbox tooltips explain the rule.

## Session 2026-06-26 — Routers page, DB listing, homepage tile, new-PC defaults, docs serving

LIVE `0c86ee9`. Migrations **058**. Builds on the FTP source (batch 3): all **4 routers**
(Brno + Zastavka + Svitavy + Jihlava) are now FTP-enabled and fetching (branches got the
`ftp` policy on `api-read-only` + the `ipscan-export` script + scheduler).

**1) "Routery / Routers" page (`NetworkPage.tsx`, nav after Tiskárny).** Surfaces the
round-trip **router → FTP → DB → page**, one card per configured router (scales to N):
FTP/REST-only + muted/stale badges, minutes since the data last advanced, parsed lease/ARP
counts, device count by source, last fetch, last error. Backed by `GET /network/routers`
(joins `site_data_status` + per-site `dhcp_leases` counts + the ftp/muted config). A
**⬇ "Stáhnout teď"** button → `POST /network/ftp-fetch` (`runFtpFetchOnce`) forces an FTP pull
of the FTP sites, **writes the parsed rows into dhcp_leases**, records `site_data_status`, and
returns a per-site **communication log** shown in a collapsible terminal console (so a branch
with no export shows its real FTP error, e.g. `530 Login incorrect`, instead of a blank card).
Under the cards a **live `dhcp_leases` listing** (`GET /network/db-rows`, up to 5000 rows) =
physical DB proof: full-text filter over all columns, click-to-sort headers (IP numeric,
last_seen by date), pagination (50/100/200/500/All), per-site filter, **CSV export**
(semicolon + BOM). New **📡 Routers homepage tile** (N/M stale; red when any monitored router
is stale; REST-only routers don't count) → opens the page; fetched on mount + the 30s refresh.

**2) Settings.** Added the missing **FTP controls** to the MikroTik block — `mikrotik.ftp_enabled`
toggle + `mikrotik.ftp_sites` field (tolerant of the `Site=IP` form, only the name before `=`
is kept). A **block filter** at the top of Settings (a `Section` hides itself when its title/
description doesn't match the query — the page got long). **New-PC defaults** (mig 058): the
"AD sync default" section gained per-flag defaults a freshly AD-discovered PC inherits — disk
monitoring, service monitoring, critical-service monitoring, excluded (all default OFF). ad-sync
applies them only on **INSERT**; existing PCs keep the operator's per-row intent.

**3) Fixes / infra.** Homepage device tiles (printers/devices/loss/ports/disks/services) were
fetched only once on mount → went **stale** until reload (the Přehled printer count disagreed
with the freshly-fetched Souhrn, 49 vs 51) → added them to the 30s refresh. **`/docs/<file>`
serving** — `/docs` only ever served `dashboard.html`; every other doc (the **bilingual
presentation**, project-status, nacitani) 404'd, so the committed presentation could never be
opened; added an allow-listed `GET /docs/:file` + a **🎞 Presentation topbar link** + the
presentation got **💾 Save HTML** (alongside 🖨 Print/PDF). Routers-page dark-theme card contrast
fixed (`var(--surface)/--text`).

**4) Data authority model (decision, recorded — see memory).** Router/DHCP = **1st authority**
for IP / network presence / online-offline (it assigns + observes the live wire); **AD = 2nd**
for identity (name/OS/type/account); **operator comment = override** on top. **Per-field**, not
blanket. **Open:** surface router-authoritative IP + a "router ≠ AD/DNS" mismatch flag on the
Computers tab (pending go).

## Session 2026-06-25 (batch 3) — MikroTik FTP file source + per-site data-freshness alert

LIVE `0cb377d`. Migrations **056–057**. New server tests (parser unit tests) — server suite **74**.
Confirmed live in prod via `GET /devices/site-status`: **Brno 96 leases + 141 ARP, no error**.

**Decision (operator):** the file source becomes the **primary** lease source, **REST stays
as a supplement until revoked** (REST still carries ip-scan NETBIOS names, which cannot be
dumped to a file). Both feed `dhcp_leases` by `(site, mac)` — no duplicates (in-memory merge
by MAC per cycle + the PK + COALESCE upsert).

**1) FTP file source (`mikrotik-ftp.ts`, mig 056).** A RouterOS scheduler on each router writes
two text files — `IP_scan.txt` (`/ip dhcp-server lease print detail file=IP_scan`) and
`ARP_scan.txt` (`/ip arp print detail file=ARP_scan`) — deleting the old file first, every 15 min.
The collector pulls them over **FTP** (the read-only `dhcp-reader` account needs the **`ftp`
policy** added to its `api-read-only` group; FTP service was already up — WinBox download uses it)
and parses both: leases (multi-line records, host-name/class-id/status) and ARP (keep **complete**
`C`-flag entries with a MAC; drop `failed`/incomplete; **join the ~72-char wrapped continuation
lines** or `status`/`vrf` are lost). No new dependency — a minimal passive-mode RETR over
`node:net`. Parsed rows merge into the same per-cycle `byMac` map (fill a missing name, add
net-new statics like the static `10.8.2.119` that DHCP leases never show). **Why files:** a
robust self-written snapshot whose header timestamp `# YYYY-MM-DD HH:MM:SS` is a per-site
**freshness signal**. Config: `mikrotik.ftp_enabled` (default 1), `mikrotik.ftp_sites` (CSV of
sites that produce the files — **Brno** only for now). New table `site_data_status` records the
file timestamps, parsed counts, `fetched_at`, `last_error`, and `file_changed_at`.

**2) Per-site data-freshness / availability alert (mig 057, `alerts.ts`).** Mirrors the printer
alert (debounce/throttle/maintenance/recipients, state in `data_freshness_alert_state`). A
monitored site (in `mikrotik.ftp_sites`, **not** in `alerts.freshness.muted_sites`) is stale when
the files stop advancing past `alerts.freshness.threshold_minutes` (default 45), can't be fetched
(`last_error`), or never produced data. **Timezone-safe:** it never compares the router's *local*
file clock to UTC — `recordSiteStatus` moves `file_changed_at` (real UTC) only when the newest
file timestamp actually increases, and the alert measures staleness from that. **Branches muted by
default** (`alerts.freshness.muted_sites='Zastavka,Svitavy,Jihlava'`) so they don't scream while
only Brno produces the files. New `POST /alerts/freshness/test`, `GET /devices/site-status`, and a
Settings section **"Notifikace — aktuálnost dat / dostupnost lokalit"** (enable/threshold/debounce/
frequency/maintenance/muted-sites/recipients/test) + CS/EN i18n.

**Router-side (manual, on 207 — done):** `ftp` policy added to `api-read-only`; script
`ipscan-export` (leases+ARP, delete-first) + scheduler `ipscan-sched` @15m. **Open for branches:**
add the same `ftp` policy + export script + scheduler on Zastavka/Svitavy/Jihlava, then add them to
`mikrotik.ftp_sites` and un-mute. **Backlog:** Phase 3 = dedicated filtered "Network" page;
Phase 4 = export ARP+ip-scan-equivalent to files so FTP can fully cover and REST can retire;
screenshot-per-deploy watcher (no repo CI — must be a `/version` watcher) with a retention policy.

## Session 2026-06-25 (batch 2) — Souhrn tab + SHR dedupe + router ip-scan names

LIVE `b370537`. No new migration (Settings + existing schema only). Full doc sweep
(README/ARCHITECTURE/dashboard.html CS+EN/project-status.html/nacitani-mac-hostname.html/HANDOFF).

**1) Manager "Souhrn" tab wired + restyled to a white managerial document.**
`ManagerSummaryPage.tsx` (WIP last session) is now in the nav (right after Přehled),
aggregating client-side over existing endpoints: equipment total + by category
(phones excluded — randomized MAC), by branch (count/PC/printers/online/offline),
printers (total/offline/low/empty supplies), PC fleet health (managed/online/problem
PCs/disk crit+warn/inactive/disabled). Restyled to the **AXIMA managerial house
style** (white, `#1d4ed8` accent, Segoe UI, KPI tiles + bar rows) and made
**theme-independent** — it previously inherited the app theme and would print/PDF
dark/unreadable in dark mode. **Tisk/PDF** and the new **Uložit HTML** both serialize
the SAME `.mr-wrap` node into one standalone UTF-8 document (Print opens it in a new
window, Save downloads it), so on-screen == printed == saved, no chrome, no mojibake.
Disk crit/warn reuse `summarizeDisks` (scope-aware, like the Dashboard); risk PCs use
`pc-health` `level==='risk'`. New i18n `nav.summary` + `summary.*` (CS+EN).

**2) Legacy `SHR-<ip>-*` shared-printer duplicates retired (self-healing).**
After host-name keying (`6da9910`), the pre-existing IP-keyed rows lingered as
ghosts that inflated the printer list. `dedupeLegacyShareKeys()` (runs each shared-
printers cycle): (a) migrates operator identity from a legacy IP-keyed row to its
modern host-name sibling (same site + printer name) when the sibling has none; (b)
deletes a legacy `SHR-%.%` lease row once a modern sibling exists, or it was never
identified, or it has no host_name (unreconcilable junk); (c) deletes ORPHANED
`device_categories` rows keyed by an old `SHR-<ip>-` mac with no surviving lease
(those were surfaced as phantom offline printers by the DB-identity UNION). Verified
live: 21 ghosts → 0; real printer count 52 → **49**; the 3 remaining `SHR-<ip>`
rows are legit (identified, offline, awaiting their host-name sibling).

**3) Router-side ip-scan as the 3rd per-router REST source (`mikrotik-collector.ts`).**
Phase 1 now fetches lease + arp **+ `POST /rest/tool/ip-scan`** (scans the router's
own /24 for `mikrotik.ipscan_duration_sec`=10s). ip-scan returns NETBIOS + reverse-DNS
+ SNMP names the lease/arp tables don't carry — and because the **router** runs it
from inside the subnet, NetBIOS resolves (it's firewalled from `.213`). Names fill any
still-nameless device by MAC (`ipScanName`: netbios short-name > reverse-DNS short >
snmp; a real DHCP host name still wins via `COALESCE`), and net-new scan devices are
added. New `routerPost`/`fetchIpScan`/`RawIpScan`; gated `mikrotik.ipscan` (default
on). The read-only `dhcp-reader` is allowed to POST ip-scan (verified live). Runs for
every configured router, concurrently with lease+arp.

> **Honest finding (don't re-chase): branch ip-scan returns no names.** Brno (.207,
> CHR 7.21) ip-scan → ~256 NETBIOS + ~53 DNS → static printers/servers get named. The
> **branch hEX routers (Zastavka .181.2 / Svitavy .182.2 / Jihlava .183.2) return
> IP+MAC but 0 NETBIOS / 0 DNS**: branch PCs have NetBIOS-over-TCP/IP disabled
> (modern AD/GPO), and the internal DNS (DC `10.8.2.254`) has **no reverse/PTR zones
> for `10.10.18x.x`** (verified: PTR for branch IPs = none; Brno `10.8.2.x` has PTR).
> Pointing the branch routers' DNS at the DC would **NOT** help — there are no PTR
> zones to resolve. Naming branch static devices needs: PTR zones on the DC, SNMP
> enabled on the branch printers (ip-scan `snmp`/sysName), or manual/AD naming.
> All 4 routers are otherwise live in the inventory (Brno 259, Zastavka 103, Jihlava
> 34, Svitavy 15 + UniFi 52).
...
- Live commit: `b370537` (+ this doc-sweep commit once deployed; verify `/version` after push)
- Schema: migrations 001–055 · Tests: 112 (Vitest) · Collectors: eventlog, disk, services, perf, reachability, ports, mikrotik (DHCP lease + ARP + **router `tool/ip-scan`** names), **unifi**, printer-supplies, shared-printers (all in-process on `.213`)
- Device sources merged by MAC: dhcp · arp · scan (app-server) · **router ip-scan (NETBIOS/DNS)** · unifi · share. All 4 MikroTik routers live (Brno 10.8.2.207 · Zastavka 10.10.181.2 · Svitavy 10.10.182.2 · Jihlava 10.10.183.2; `dhcp-reader`/`DHCPApi`).
